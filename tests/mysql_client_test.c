...
static void test_bug21199582()
{
  ...
  for (; i < 2; ++i)
  {
    ...
    myquery(mysql_query(mysql,
                        "insert into abcd values(1,'a'),(2,'b'),(3,'c')"));

    if(i)
    {
      sprintf(query,"create procedure p3(INOUT p1 Integer) BEGIN "
                    "select c from abcd;insert into abcd values(4,'d');"
                    "select id from abcd;"
                    "SET NAMES 'utf8';set autocommit = ON;SET p1 = 9999;END");
    }
    else
    {
      sprintf(query,"create procedure p3(INOUT p1 Integer) BEGIN "
              "select id from abcd;insert into abcd values(4,'d');"
              "select id from abcd;SET NAMES 'utf8';set autocommit = ON;"
              "SET p1 = 9999;END");
    }

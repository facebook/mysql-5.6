 ...
 *
 * Value of `*result` is `true` when an error occurs.
 *
 * Returns net_async_status
 */
static int run_ssl_connect(MCPVIO_EXT *mpvio, my_bool init_fd, my_bool *result) {
                                    ...
                                    const uchar *data, int data_len)
{
  ...
  */
  if (mysql->client_flag & CLIENT_SSL)
  {
    ...
    my_bool error = FALSE;
    net_async_status status = run_ssl_connect(mpvio, TRUE, &error);
    DBUG_ASSERT(status == NET_ASYNC_COMPLETE);
    if (error) {
      my_free(buff);
      DBUG_RETURN(1);
    }
    ...
    MCPVIO_EXT *mpvio,const uchar *pkt, int pkt_len, my_bool *result)
{
  ...
  if (ctx->ssl_exchange_state == SSL_CONNECT) {
    ...
    net_async_status status =
      run_ssl_connect(mpvio, first_call, &error);
    if (status == NET_ASYNC_NOT_READY) {
      DBUG_RETURN(NET_ASYNC_NOT_READY);
    }
    if (error) {

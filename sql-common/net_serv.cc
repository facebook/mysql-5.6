...
static net_async_status net_read_compressed_nonblocking(NET *net,
  ...
  if (net_async->mp_status != NET_ASYNC_NOT_READY)
    net_read_init_offsets(
        net, net_async->mp_start_of_packet, net_async->mp_first_packet_offset,
        net_async->mp_multi_byte_packet, net_async->mp_buf_length);

  for (;;) {
    /*  Read the current packet in net->buff */
    if (net_read_process_buffer(
            net, net_async->mp_start_of_packet, net_async->mp_buf_length,
            net_async->mp_multi_byte_packet, net_async->mp_first_packet_offset))
      break;
    ...
    */
    ...
    if (net_async->mp_status == NET_ASYNC_NOT_READY) {
      net->save_char = net->buff[net_async->mp_first_packet_offset];
      net->buf_length = net_async->mp_buf_length;
      return net_async->mp_status;
    }

    if (len == packet_error) {
      net_async->mp_status = NET_ASYNC_COMPLETE;
      return net_async->mp_status;
    }
    net_async->mp_buf_length += len;
  }
  /*
    Once the packets are read in the net->buff, adjust the tracking offsets to
    the appropriate values.
  */
  len = net_read_update_offsets(
      net, net_async->mp_start_of_packet, net_async->mp_first_packet_offset,
      net_async->mp_buf_length, net_async->mp_multi_byte_packet);
  net_async->mp_status = NET_ASYNC_COMPLETE;
  return net_async->mp_status;
}
...
static net_async_status net_read_uncompressed_nonblocking(NET *net,
  ...
  if (net_async->mp_status == NET_ASYNC_COMPLETE) {
    net_async->mp_save_pos = net->where_b;
    net_async->mp_total_length = 0;
  }
  ...
  if (len == MAX_PACKET_LENGTH) net_async->mp_status = NET_ASYNC_NOT_READY;
  if (net_async->mp_status == NET_ASYNC_NOT_READY) return net_async->mp_status;

  // Update the offsets
  net->where_b = net_async->mp_save_pos;
  len = net_async->mp_total_length;
  net->read_pos = net->buff + net->where_b;
  net_async->mp_status = NET_ASYNC_COMPLETE;
  return net_async->mp_status;
}

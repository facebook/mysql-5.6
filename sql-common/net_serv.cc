  ...
  for (size_t packet_num = 0; packet_num < packet_count; ++packet_num) {
    ...
    */
    ...
    if (packet_num == 0 && optional_prefix != NULL) {
      if (try_coalesce && vec->iov_len + prefix_len <= ASYNC_COALESCE_BUFFER_SIZE) {
        memcpy((uchar*) vec->iov_base + vec->iov_len, optional_prefix, prefix_len);
    ...
    */
    ...
    if (remaining_bytes != 0) {
      ...
      if (try_coalesce && vec->iov_len + remaining_bytes <= ASYNC_COALESCE_BUFFER_SIZE) {

...
sig_handler mysql_end(int sig)
{
  ...
  */
  ...
  if (alias_list)
  {
    for( i = 0; alias_list[i]->alias && !found_null; i++ )
    {
...
void push_alias(char *alias, char *cmd)
{
  ...
  struct alias_listitem *null_alias;
  int cnt=0;

  if ( alias )
  {
    if ( !alias_list )
      push_alias(NULL, NULL); // seed the null, since it's missing

    // Find the end.
    for ( cnt = 1; alias_list[cnt-1]->cmd; cnt++ )
      ;
  }

  if ( !( alias_list = (struct alias_listitem **) my_realloc(alias_list,
          sizeof(alias_list)*(cnt+1),
          MYF(MY_FAE)) ) )
  {
  ...
  alias_list[(alias ? cnt-1 : 0)] = new_alias;
}
...
void read_alias_dir(const char *dir, int recursion_level)
{
  ...
  /* ARGSUSED */
static int
com_runalias(String *buffer __attribute__((unused)),
    char *line)
{
  char *tmp, buff[256];
  int i;
   ...
   * empty list, this will only be run once, because we'll add the NULL root
   * here.
   */
  if (!alias_list)
  {
    push_alias(NULL, NULL);

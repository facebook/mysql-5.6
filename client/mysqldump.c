...
#include <sslopt-longopts.h>
  ...
  {"long_query_time", OPT_LONG_QUERY_TIME,
   ...
   GET_ULONG, REQUIRED_ARG, 86400, 0, LONG_TIMEOUT, 0,
   0, 0},
  {"timeout", OPT_TIMEOUT,
                                ...
                                char *ignore_flag)
{
  ...
  *ignore_flag= check_if_ignore_table(table, table_type);
  ...
  if (opt_order_by_primary || opt_order_by_primary_desc)
    order_by= primary_key_fields(result_table, opt_order_by_primary_desc? TRUE : FALSE);
    ...
    */
    if (!opt_view_error)
    {
      while ((table= getTableName(1)))
      {
        ...
        sprintf(query, "LOCK TABLES %s READ /*!32311 LOCAL */",
            quote_name(table, table_buff, 1));
        /* Only views have definers. Add view to ignore list. */
        if (mysql_query(mysql, query) &&
            mysql_errno(mysql) == ER_NO_SUCH_USER)
        {
          strmov(afterdot, table);
          fprintf(stderr, "Warning: Could not lock view %s because of "
                             ...
                             char* pos_out, char** gtid_executed_set_pointer)
{
  ...
  */
  ...
  {
    MYSQL_RES *res = NULL;
    const char* command_innodb=  "START TRANSACTION WITH CONSISTENT INNODB SNAPSHOT";
    const char* command_rocksdb= "START TRANSACTION WITH CONSISTENT ROCKSDB SNAPSHOT";
...
static char *primary_key_fields(const char *table_name, const my_bool desc)
{
  ...
  if ((row= mysql_fetch_row(res)) && atoi(row[1]) == 0)
  {
    ...
    do
    {
      ...
      if (desc)
      {
        result_length+= 5; /* + 1 for space, and + 4 for "DESC" */
      }

...
/*****************************************************************************
 * This function waits for IO, to simulate a blocking call.
 ****************************************************************************/
#include <sys/time.h>
...
/*
  ...
  Sets $var to be a string of "X" repeated 'size' times.
*/

void do_pad_variable(struct st_command *command)
{
  DBUG_ENTER(__func__);
  int size;
  char *buf;
  VAR *v;
  static DYNAMIC_STRING var_name;
  static DYNAMIC_STRING var_size;
  const struct command_arg pad_var_args[] = {
    { "var_name", ARG_STRING, TRUE, &var_name, "variable to pad" },
    { "var_size", ARG_STRING, TRUE, &var_size, "size to pad to" }
  };

  check_command_args(command, command->first_argument,
                     pad_var_args,
                     sizeof(pad_var_args)/sizeof(struct command_arg),
                     ' ');

  size = atoi(var_size.str);
  buf = (char*)my_malloc(size, MYF(MY_WME));
  memset(buf, 'X', size);
  DBUG_PRINT("info", ("Pad %s to %d", var_name.str, size));
  v = var_obtain(var_name.str, var_name.length);
  var_init(v, var_name.str, var_name.length, buf, size);
  dynstr_free(&var_name);
  dynstr_free(&var_size);
  DBUG_VOID_RETURN;
}

...
bool Master_info::write_info(Rpl_info_handler *to)
{
  ...
  */
  ...
  else
  {
    ...
    if (ignore_server_ids->pack_dynamic_ids(&buffer))
      DBUG_RETURN(TRUE);
    if (to->set_info(LINES_IN_MASTER_INFO, "%d\n%s\n%lu\n%s\n%s\n%s\n%u\n%u\n"
                     "%d\n%s\n%s\n%s\n%s\n%s\n%d\n%s\n%s\n%s\n%s\n%lu\n%s\n"
                     "%s\n%d\n",
                     ...
                     hb_period, bind_addr, buffer.c_ptr_safe(),
                     master_uuid, retry_count, ssl_crl, ssl_crlpath,
                     (int) auto_position))
      DBUG_RETURN(TRUE);
  }
  DBUG_RETURN(FALSE);

...
void THD::cleanup_after_query() {
  ...
  */
  if (!sp_runtime_ctx) m_trans_fixed_log_file = m_trans_fixed_log_path = NULL;
...
void THD::serialize_client_attrs() {
  ...
  for (size_t i = 0; i < client_attrs.size(); i++) {
    ...
    if (i > 0) {
      q_append(C_STRING_WITH_LEN(", "), &buf);
    }
    q_append('\'', &buf);
    q_append(p.first.ptr(), MY_MIN(100, p.first.length()), &buf);
    q_append(C_STRING_WITH_LEN("' : '"), &buf);
    q_append(p.second.ptr(), MY_MIN(100, p.second.length()), &buf);
    q_append('\'', &buf);
  }
  q_append('}', &buf);

...
void THD::kill_query_with_error(uint error, ...) {
  ...
  DBUG_ASSERT(!deferred_error.is_set());
...
bool THD::fillMetadata(struct st_ok_metadata &metadata) {
  ...
  if (tracker && tracker->is_enabled() && tracker->is_changed()) {
    DBUG_ASSERT(dynamic_cast<Session_resp_attr_tracker *>(tracker));
  ...
  if (tracker && tracker->is_enabled() && tracker->is_changed()) {
    DBUG_ASSERT(dynamic_cast<Session_state_change_tracker *>(tracker));
  ...
  if (tracker && tracker->is_enabled() && tracker->is_changed()) {
    DBUG_ASSERT(dynamic_cast<Session_gtids_tracker *>(tracker));

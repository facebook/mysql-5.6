  ...
  for (const auto &pair : db_ac->ac_map) {
    ...
    while (iter.move()) {
      ...
      static std::array<std::string, 4> errors = {"UNKNOWN", "ABORTED",
                                                  "TIMEOUT", "KILLED"};
      DBUG_ASSERT(static_cast<ulong>(r.res) < errors.size());
      ...
      static std::array<std::string, 4> modes = {"UNKNOWN", "QUERY", "LOW_PRI",
                                                 "HIGH_PRI"};
      DBUG_ASSERT(static_cast<ulong>(r.mode) < modes.size());
...
void Ac_info::log_error(THD *thd, Ac_result res, ulong queue,
  ...
  // they wrap around.
  DBUG_ASSERT(static_cast<long>(r.queue_boosts) >= 0);
  DBUG_ASSERT(static_cast<long>(r.exits) >= 0);

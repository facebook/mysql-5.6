...
int rli_relay_log_raft_reset(
  ...
  */
  ...
  // up to date
  if (mi->rli->relay_log.open_existing_binlog(
        opt_bin_logname, max_binlog_size)) {
    // NO_LINT_DEBUG
...
int rotate_relay_log(Master_info *mi, bool log_master_fd, bool need_lock,
  ...
  */
  ...
  if (log_master_fd)
    error =
        rli->relay_log.new_file_without_locking(
...
int rotate_relay_log_for_raft(
    const std::string& new_log_ident, ulonglong pos, myf raft_flags) {
  ...
  error = rotate_relay_log(
      mi,
      /*log_master_fd=*/ true,
      /*need_lock=*/ true,
      /*need_log_space_lock=*/ true,
      raft_flags);

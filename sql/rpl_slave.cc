...
static int queue_event(Master_info* mi,const char* buf, ulong event_len)
{
  ...
  /*
    ...
    got accidentally set to be a non-linear value somehow. In other words, we
    need this variable to make sure the timestamp of each event in
    the binlog network stream can be updated correctly when the master's
    timestamp is set non-linear. Each binlog event's timestamp is updated
    accordingly before being written into the relay log.
  */
  ...
  */
  ...
  /*
    ...
    the non-linearly set timestamp of the events from the master when necessary.
  ...
  */
  if (cur_event_timestamp != 0)
  {
    ...
    */
    ...
    /*
      For all the following events, if the timestamp is non-linearly set as,

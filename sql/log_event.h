...
#if defined(MYSQL_SERVER)
  int pack_info(Protocol *protocol) override;
  void *setup_table_rli(RPL_Table_ref **table_list);
#endif
...
class Rows_log_event : public virtual binary_log::Rows_event, public Log_event {
 ...
 private:
  ...
  void close_table_ref(RPL_Table_ref *table_list);
  virtual void prepare_dep(Relay_log_info *rli,
                           std::shared_ptr<Log_event_wrapper> &ev) override;
  int do_apply_event(Relay_log_info const *rli) override;
  int do_update_pos(Relay_log_info *rli) override;
  ...
  */
  virtual int do_before_row_operations(const Relay_log_info *const log,
...
#if defined(MYSQL_SERVER)
  int do_before_row_operations(const Relay_log_info *const,
                               table_def *) override;

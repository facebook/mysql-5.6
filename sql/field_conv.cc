...
static type_conversion_status do_field_6(Copy_field *copy)
{						// For blob field
  copy->to_ptr[0]=copy->from_ptr[0];
  copy->to_ptr[1]=copy->from_ptr[1];
  copy->to_ptr[2]=copy->from_ptr[2];
...
static type_conversion_status do_save_doc(Copy_field *copy){
  ...
  {
    ret= copy_doc_field((Field_document*)copy->from_field,
...
type_conversion_status set_field_to_null(Field *field)
{
  ...
  {
    DBUG_ASSERT(false); // non-nullable document field shouldn't exist
    // we don't have default value for document field,
    // so disallow null value for non-nullable document field
...
static type_conversion_status do_conv_blob(Copy_field *copy)
{
  ...
  return ((Field_blob *) copy->to_field)->store(copy->tmp.ptr(),
					 copy->tmp.length(),
					 copy->tmp.charset());
}
...
{
  type_conversion_status status= TYPE_OK;
  char buff[MAX_FIELD_WIDTH];
...
{
  my_decimal value;
  return copy->to_field->store_decimal(copy->from_field->val_decimal(&value));
}
...
static type_conversion_status do_cut_string_complex(Copy_field *copy)
...
copy_field_varstring(Field_varstring * const to,
                     const Field_varstring * const from)
{
  ...
  const uint bytes_to_copy= get_varstring_copy_length(to, from, &truncated);
  if (length_bytes == 1)
...
{
  return copy_field_varstring(static_cast<Field_varstring*>(copy->to_field),
...
type_conversion_status field_conv(Field *to,Field *from)
{
      ...
      to->charset() == from->charset())
  {
        ...
        from->real_type() == MYSQL_TYPE_VARCHAR)
    {
      ...
      {
        return copy_field_varstring(to_vc, from_vc);
      }
    }

...
#define MYSQL_SEMANTIC_YYABORT_UNLESS(A,B)         \
  if (!(A))                                        \
  {                                                \
    my_parse_semantic_error(ER(ER_SYNTAX_ERROR),B);\
    MYSQL_YYABORT;                                 \
  }
...
/**
  ...
  This function provides semantic action implementers with a way
  to push the famous "You have a syntax error near..." error
  message into the error stack, for semantic errors not discovered
  internally by the Bison generated parser, by passing the position
  information explicitly to the function.
*/

void my_parse_semantic_error(const char *s, const char *yytext)
{
  THD *thd= current_thd;
  Lex_input_stream *lip= & thd->m_parser_state->m_lip;
...
%union {
  ...
  String *string;
  Document_path_key_spec_type *key_part_type;
...
%type <NONE>
        ...
        view_check_option np_tail trigger_tail sp_tail sf_tail udf_tail event_tail
...
create:
        ...
        | create_opt opt_unique_combo_clustering INDEX_SYM ident key_alg ON table_ident
          {
...
key_def:
        ...
        | opt_constraint constraint_key_type opt_ident key_alg
          ...
          {
            if (($1.length != 0)
                && ((enum Key::Keytype)$2 == (Key::CLUSTERING |
                                              Key::MULTIPLE)))
            {
...
key_part_type:
          INT_SYM
          {
            $$ = new Document_path_key_spec_type(MYSQL_TYPE_LONGLONG, 8);
            if ($$ == NULL)
              MYSQL_YYABORT;
          }
        | DOUBLE_SYM
          {
            $$ = new Document_path_key_spec_type(MYSQL_TYPE_DOUBLE, 8);
            if ($$ == NULL)
              MYSQL_YYABORT;
          }
        | BOOL_SYM
          {
            $$ = new Document_path_key_spec_type(MYSQL_TYPE_TINY, 1);
            if ($$ == NULL)
              MYSQL_YYABORT;
          }
        | STRING_SYM '(' NUM ')'
        ...
        | dot_separated_ident AS key_part_type
          {
            ...
            {
              list.push_back(&ident->s);
              if (idx == 0)
                 field_name = ident->s;
            }
            Document_path_key_spec spec(list, *$3);
...
commit:
	    ...
	    MYSQL_SEMANTIC_YYABORT_UNLESS($3 != TVL_YES || $4 != TVL_YES
	                                  || $5 != TVL_YES, "RELEASE");

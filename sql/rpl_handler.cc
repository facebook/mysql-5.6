...
int Raft_replication_delegate::get_applied_opid_set(std::string *opid_set_str) {
  DBUG_ENTER("Raft_replication_delegate::get_applied_opid_set");
  int ret = 0;
  FOREACH_OBSERVER_STRICT(ret, get_applied_opid_set, (opid_set_str));
  DBUG_RETURN(ret);
}

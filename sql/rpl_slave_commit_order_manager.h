                                                ...
                                                THD *thd_wait_for)
{
  ...
  {
    DBUG_ASSERT(self_w->c_rli->mts_dependency_replication != DEP_RPL_TABLE);
    if (self_w->c_rli &&
        self_w->c_rli->mts_dependency_replication == DEP_RPL_TABLE)
    {
      // NO_LINT_DEBUG
      sql_print_error("InnoDB lock wait callback fired in TBL mode!");
    }

    DBUG_PRINT("info", ("Found slave order commit deadlock"));
    // NO_LINT_DEBUG
    sql_print_warning(
        "Commit order deadlock between: self = %u (worker id %lu), waiting for "
        "= %u (worker id %lu). Will rollback and retry trx on thread %u.",
        thd_self->thread_id(), self_w->id, thd_wait_for->thread_id(),
        wait_for_w->id, thd_wait_for->thread_id());

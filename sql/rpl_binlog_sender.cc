...
bool Binlog_sender::get_dscp_value(int &ret_val) {
  ...
  const char *tmp = (char *)(dscp_str + MY_MIN(3, dscp_it->second.length()));
  int res = 0;
...
void Binlog_sender::run() {
  ...
  while (!has_error() && !m_thd->killed) {
    ...
    */
    ...
    int error = raw_log->find_next_log(&m_linfo, 0);
    raw_log->unlock_index();
    if (unlikely(error)) {
      DBUG_EXECUTE_IF("waiting_for_disable_binlog", {
...
int Binlog_sender::send_events(File_reader *reader, my_off_t end_pos) {
  ...
  while (likely(log_pos < end_pos) || end_pos == 0) {
    ...
    */
    if (m_exclude_gtid &&
      ...
      const
#endif
          bool time_for_hb_event =
              (diff_timespec(&cur_clock, &m_last_event_sent_ts) >=
               m_heartbeat_period);

      DBUG_EXECUTE_IF("send_zero_hb_event", { time_for_hb_event = true; });

      if (time_for_hb_event) {
        if (unlikely(send_heartbeat_event(log_pos, false))) DBUG_RETURN(1);
  ...
  */
  if (unlikely(in_exclude_group)) {
    if (send_heartbeat_event(log_pos, false)) DBUG_RETURN(1);
  }
  DBUG_RETURN(0);
}
  ...
  if (m_start_file[0] != '\0') {
    ...
    */
    ...
    if (!lost_gtids->is_subset(m_exclude_gtid)) {
      Gtid_set gtid_missing(lost_gtids->get_sid_map());
      gtid_missing.add_gtid_set(lost_gtids);

                                       ...
                                       const std::vector<Session_var>& vars)
{
  ...
  for(uint i = 0; i < vars.size(); ++i)
  {
    ...
    if (!validate_val(vars[i].first, vars[i].second))
      return false;
  ...
  for(uint i = 0; i < users.size(); ++i)
  {
    std::pair<User_session_vars_it, bool> ret =
      per_user_vars->insert(std::make_pair(users[i], sp_vars));
                                         ...
                                         const char *sys_var_str)
{
  ...
  */
  while (*p)
  {
    ...
    while (*p && !strchr(delimiters, *p) && !strchr(invalid_tokens, *p))
      ++p;

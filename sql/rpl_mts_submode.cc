 ...
 * @return void
 */
void Mts_submode_dependency::wait_for_dep_workers_to_finish(
...
void Mts_submode_dependency::add_row_event(
  ...
  // sync mode!
  if (unlikely(!rle->get_keys(rli, ev, keylist) ||
...
std::shared_ptr<Log_event_wrapper> Mts_submode_dependency::dequeue(
  ...
  if (ret && co_mngr != nullptr) {
    DBUG_ASSERT(rli->mts_dependency_order_commits);
...
void Mts_submode_dependency::enqueue(
  ...
  if (unlikely(dep_queue.size() >= rli->mts_dependency_size)) {
    dep_full = true;
  }
...
*/
void Mts_submode_dependency::schedule_dep(Relay_log_info *rli, Log_event *ev) {
  ...
  if (unlikely(dep_sync_group)) wait_for_dep_workers_to_finish(rli, trx_queued);
...
void Mts_submode_dependency::handle_terminal_event(
    Relay_log_info *rli, std::shared_ptr<Log_event_wrapper> &evw) {
...
void Mts_submode_dependency::signal_trx_done(
  ...
  if (num_in_flight_trx <= 1) mysql_cond_signal(&dep_trx_all_done_cond);
  mysql_mutex_unlock(&dep_lock);

...
static int binlog_recover(Binlog_file_reader *binlog_file_reader,
                          ...
                          char *engine_binlog_file, my_off_t *engine_binlog_pos,
  ...
  if (ev != nullptr) {
    ...
    if (write_meta_data_event) {
      ...
      Rows_query_log_event metadata_ev(thd, metadata.c_str(),
                                       metadata.length());
      if (binary_event_serialize(&metadata_ev, &m_cache) != 0) DBUG_RETURN(1);
    }
...
int MYSQL_BIN_LOG::new_file(
  ...
  return new_file_impl(
      true /*need_lock_log=true*/, extra_description_event, raft_flags);
}
...
int MYSQL_BIN_LOG::new_file_without_locking(
    Format_description_log_event *extra_description_event, myf raft_flags) {
  return new_file_impl(
      false /*need_lock_log=false*/, extra_description_event, raft_flags);
}
  ...
  if (m_binlog_file->is_open() && !skip_re_append) {
    ...
    */
    ...
    if (rotate_via_raft) {
      ...
      if ((error = temp_binlog_cache->open(4000, 4000)))
        goto end;

      if ((error = r.write(temp_binlog_cache.get())))
        goto end;


      add_bytes_written(((Log_event *)&r)->common_header->data_written);
    } else if (DBUG_EVALUATE_IF(
          "fault_injection_new_file_rotate_event", (error = 1), false) ||
    ...
    while ((ev = binlog_file_reader->read_event_object())) {
      if (ev->get_type_code() == binary_log::QUERY_EVENT &&
        ...
        if (ev->get_type_code() == binary_log::XID_EVENT) {
          ...
          current_xid = qev->ddl_xid;
        }

        if (!xids.emplace(current_xid, current_gtid).second) goto err1;
      }

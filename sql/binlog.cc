...
// Print the read-only flag changes to log for visibility
void print_read_only_change(THD *thd)
{
  ...
  // NO_LINT_DEBUG
  sql_print_information(
    "Setting global variable: read_only = %d , (user '%s' from '%s')",
    opt_readonly,
    user,
    host
    );
...
// Checks conditions before updating read-only flag
bool before_set_read_only(THD *thd, ulonglong read_only)
{
  ...
  if (!read_only && is_slave && read_only_slave)
  {
...
// Updates the read-only flag in the server based on 'readonly'.
// Much has been borrowd from sys_vars.cc::fix_read_only
bool set_read_only(THD* thd, ulonglong readonly)
{
  DBUG_ENTER("set_read_only");
  bool result= true;

  // Lock global system variables to prevent concurrent changes here

...
int Log_event::apply_event(Relay_log_info *rli) {
  ...
  if (!(parallel = rli->is_parallel_exec()) ||
    ...
    if (is_row_log_event() && rli->last_gtid[0] != '\0' &&
      ...
      if (current_gtid.sidno == rli->recovery_max_engine_gtid.sidno &&
        ...
        auto casted_this = static_cast<Rows_log_event *>(this);
        casted_this->m_force_binlog_idempotent = true;
      }
    }
  ...
  */
  ...
  if (table) {
    ...
    */
    ...
    // Yura: consider removing this if completely
    if (!thd->slave_thread)  // set the mode for user thread
    ...
    if (m_force_binlog_idempotent && !error) {
      ...
      if (table->file->write_locked_table_maps(thd))
        ...
        error = mysql_bin_log.write_event(
            this, table->file->has_transactions()

  ...
  if (seq_execution)
  {
    ...
    DBUG_EXECUTE_IF("dbg_enable_idempotent_recovery", {
       if (is_row_log_event())
       {
          ...
          current_gtid.parse(rli->recovery_sid_map, rli->last_gtid);
                                                 ...
                                                 std::string& err_msg) const
{
  ...
  {
    err_msg= "Found row event with incomplete column information. Check value "
  ...
  {
    err_msg= "Found table without primary key while performing idempotent "
             "recovery: " + table_name + ". Log position: " + pos;
    return false;
  }
  return true;
}
...
int Update_rows_log_event::force_update(Relay_log_info const *rli, const uchar *const curr_row)
{
  int error= 0;
  DBUG_ASSERT(m_table);
  ...
  error= unpack_row(rli, m_table, m_width, row_start, &m_cols,
                    const_cast<const uchar**>(&row_end),
                    &m_master_reclength, m_rows_end);
  row_start= row_end;
  ...
  while (!error && (error= m_table->file->ha_write_row(m_table->record[0])))
  {
    ...
    else
    {
      ...
      if (key.get() == NULL)
      {
        key.assign(static_cast<char*>(
              my_alloca(m_table->s->max_unique_length)));
        if (key.get() == NULL)
          return ENOMEM;
      }
      ...
      error= m_table->file->ha_index_read_idx_map(m_table->record[1], keynum,
                                                  (const uchar*)key.get(),
                                                  HA_WHOLE_KEY,
                                                  HA_READ_KEY_EXACT);
    }
  ...
  {
    sql_print_error("Got an idempotent error during "
                    "Update_rows_log_event::force_update(), "
                    "error: %d, pos: %s:%llu",
                    error, rli->get_rpl_log_name(), log_pos);
    error= HA_ERR_GENERIC;
  }
  return error;
}

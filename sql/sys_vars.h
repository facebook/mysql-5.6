...
/**
  Class for @@applied_opid_set
*/
class Sys_var_applied_opid_set : public sys_var {
 public:
  Sys_var_applied_opid_set(
      const char *name_arg, const char *comment, int flag_args, ptrdiff_t off,
      size_t, CMD_LINE getopt, const char *def_val, PolyLock *lock = nullptr,
      enum binlog_status_enum binlog_status_arg = VARIABLE_NOT_IN_BINLOG,
      on_check_function on_check_func = nullptr,
  ...
  bool global_update(THD *, set_var *var) override {
    DBUG_TRACE;
    return !mysql_bin_log.set_applied_opid_set(
        var->save_result.string_value.str);
  }
  ...
  const uchar *global_value_ptr(THD *thd, std::string_view) override {
    DBUG_TRACE;
    std::string opid_set;
    mysql_bin_log.get_applied_opid_set(&opid_set);
    char *buf = (char *)thd->alloc(opid_set.length() + 1);
    if (buf == nullptr)
      my_error(ER_OUT_OF_RESOURCES, MYF(0));
    else
      std::strcpy(buf, opid_set.c_str());
    return (uchar *)buf;
  }

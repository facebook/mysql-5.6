...
/**
   ...
   2. Increments waiting_queries. Error out if we crossed the max limit.
...
bool DB_AC::admission_control_enter(THD* thd, const std::string& db) {
  ...
  if (++it->second->running_queries <= max_running_queries.load()) {
  ...
  // Waiting mode.
  if (++it->second->waiting_queries > max_waiting_queries.load() &&
      max_waiting_queries.load()) {
  ...
  */
  ...
  do
  {
    ...
    // Need to check we are below max running limit before letting this thread
    // to actually run.
  } while (++it->second->running_queries > max_running_queries.load() &&
           max_running_queries.load() && !thd->killed &&
           !it->second->abort_waiting.load());
          ...
          */
                       ...
                       lex->sql_command != SQLCOM_SET_OPTION))
          {
            ...
            */
            if (!(thd->security_ctx->master_access & SUPER_ACL) &&
                !thd->rli_slave &&
                thd->db && max_running_queries.load()) {
            ...
            if (admission_check &&
              ...
              my_error(ER_DB_ADMISSION_CONTROL, MYF(0),
                       max_waiting_queries.load(), thd->db);
              error= 1;
            } else {
            error=

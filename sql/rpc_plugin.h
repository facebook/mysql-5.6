...
const int MAX_VALUES_PER_RPC_WHERE_ITEM = 30;
...
struct myrocks_column_metadata {
  std::string name;
  myrocks_value_type type;
};
...
/*
   myrocks calls this callback function when the metadata of resultset is ready.
   rpc_buffer   : points to protocol-specific (eg. thrift) rpc buffer where
                  metadata in result are converted to protocol-specific format
                  and stored.
   result       : points to where column metadata are stored.
*/
using myrocks_bypass_rpc_send_metadata_fn =
    void (*)(void *rpc_buffer, std::vector<myrocks_column_metadata> *result);
...
struct myrocks_select_from_rpc {
  ...
  myrocks_bypass_rpc_send_metadata_fn send_metadata;
};

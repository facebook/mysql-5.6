...
/*
 * Returns the THD corresponding to the session where the query is executed.
 * If there is an srv_session attached, it returns the srv_session thd.
 * Otherwise it returns the conn_thd.
 * Function is called with locks acquired in conn_thd, so if there is a
 * srv_session attached, it will release the locks in conn_thd and acquire them
 * in session thd.
 * */
static THD* get_session_thd_locked(THD* conn_thd, THD* srv_session_thd,
                                   struct st_my_thread_var *&mysys_var)
{
  ...
  {
    // unlock mutexes in conn_thd
    if (mysys_var)
      mysql_mutex_unlock(&mysys_var->mutex);

    mysql_mutex_unlock(&conn_thd->LOCK_thd_data);

    // acquire srv_session_thd mutexes
    mysql_mutex_lock(&srv_session_thd->LOCK_thd_data);

    if ((mysys_var = srv_session_thd->mysys_var) != nullptr)
      mysql_mutex_lock(&mysys_var->mutex);

    return srv_session_thd;
  }
}
...
static void set_thread_info_common(thread_info *thd_info, THD *thd,
                                   THD *conn_thd, ulong max_query_length)
{
  struct st_my_thread_var *mysys_var;
  Security_context *tmp_sctx= conn_thd->security_ctx;
  ...
  if (conn_thd->peer_port && (tmp_sctx->get_host()->length() ||
            tmp_sctx->get_ip()->length()) && thd->security_ctx->host_or_ip[0])
  {
    if ((thd_info->host= (char*) thd->alloc(LIST_PROCESS_HOST_LEN+1)))
      my_snprintf((char *) thd_info->host, LIST_PROCESS_HOST_LEN,
        "%s:%u", tmp_sctx->host_or_ip, conn_thd->peer_port);
  }
  else
  ...
  DBUG_EXECUTE_IF("processlist_acquiring_dump_threads_LOCK_thd_data", {
    if (conn_thd->get_command() == COM_BINLOG_DUMP ||
        conn_thd->get_command() == COM_BINLOG_DUMP_GTID)
      DEBUG_SYNC(thd, "processlist_after_LOCK_thd_count_before_LOCK_thd_data");
  });
  /* Lock THD mutex that protects its data when looking at it. */
  mysql_mutex_lock(&conn_thd->LOCK_thd_data);

  thd_info->command= get_session_command(conn_thd);
  ...
  if ((mysys_var= conn_thd->mysys_var))
    mysql_mutex_lock(&mysys_var->mutex);
  thd_info->proc_info =
      (char *)(conn_thd->killed == THD::KILL_CONNECTION ? "Killed" : 0);

  // rest of the information will be collected from session thd
  auto session_thd = get_session_thd_locked(conn_thd, srv_session_thd,
                                            mysys_var);

  thd_info->state_info= thread_state_info(session_thd);
  if (mysys_var)
    mysql_mutex_unlock(&mysys_var->mutex);

  set_thread_info_query_safe(thd_info, thd, session_thd, max_query_length);
                                         ...
                                         THD *thd)
{
  ...
  if (thd->variables.high_precision_processlist)
    field_list->push_back(new Item_float(NAME_STRING("Time"),0,6,65));
  else
  {
                                           ...
                                           THD *thd)
{
  ...
  timeval tm_delta;
  if (thd->variables.high_precision_processlist &&
      (thd_info->start_time.tv_sec || thd_info->start_time.tv_usec))
  {
    char buff[65]; // buffer to hold the double field with max length 65
    String tmp_str(buff, sizeof(buff), system_charset_info);

    my_timeval_minus(time_now, &(thd_info->start_time), &tm_delta);
    protocol->store(my_timeval_to_double(&tm_delta), 6, &tmp_str);
  }
  else if (thd_info->start_time.tv_sec)
  {
    my_timeval_minus(time_now, &(thd_info->start_time), &tm_delta);
    protocol->store_long(my_timeval_to_longlong(&tm_delta));
  }
  else
    protocol->store_null();
                                       ...
                                       struct st_my_thread_var *&mysys_var)
{
  ...
  if ((val= (char *) (tmp->killed == THD::KILL_CONNECTION? "Killed" : 0)))
    ...
    auto command = get_session_command(tmp);
    table->field[4]->store(command_name[command].str,
                           command_name[command].length, cs);
  }
  return true;
}
                                    ...
                                    timeval *time_now, bool key_field_id)
{
  ...
  /* MYSQL_TIME */
  timeval *tm = srv_session_thd ?
                &srv_session_thd->start_time : &conn_thd->start_time;
  timeval tm_delta;
  my_timeval_minus(time_now, tm, &tm_delta);
  if (thd->variables.high_precision_processlist)
    table->field[5]->store(my_timeval_to_double(&tm_delta));
  else
    table->field[5]->store(my_timeval_to_longlong(&tm_delta), FALSE);
                                         ...
                                         char *user, CHARSET_INFO *cs)
{
  ...
  // LOCK_thd_data is already acquired by fill_fields_process_common
  ...
  // all other info will be collected from session thd
  THD* tmp = get_session_thd_locked(conn_thd, srv_session_thd, mysys_var);
                                 ...
                                 ulong max_query_length)
{
  ...
  thd_info->start_time= session_thd->start_time;
                                    ...
                                    process_list_type type)
{
  ...
  ulong max_query_length= (verbose ? thd->variables.max_allowed_packet :
			   PROCESS_LIST_WIDTH);
  Protocol *protocol= thd->protocol;
  const char *dbug_name MY_ATTRIBUTE((__unused__));
  switch (type)
  {
  ...
  switch (type)
  {
    ...
    case process_list_type::SHOW_PROCESS_LIST:
      setup_field_list_processlist(&field_list, max_query_length, thd);
      field_list.push_back(
        new Item_int(NAME_STRING("Srv_Id"), 0, MY_INT64_NUM_DECIMAL_DIGITS));
      break;
  ...
  timeval time_now;
  my_micro_time_to_timeval(my_micro_time(), &time_now);
  for (size_t ix= 0; ix < thread_infos.size(); ++ix)
  {
    ...
    switch (type)
    {
      ...
      case process_list_type::SHOW_PROCESS_LIST:
        store_result_field_processlist(protocol, thd_info, &time_now, thd);
        break;

      case process_list_type::SHOW_SRV_SESSIONS:
        store_result_field_srv_sessions(protocol, thd_info, &time_now, thd);
        break;
...
ST_FIELD_INFO user_latency_histograms_fields_info[]=
{
  {"USER_NAME", NAME_LEN, MYSQL_TYPE_STRING, 0, 0, 0, SKIP_OPEN_TABLE},
  {"TYPE", NAME_LEN, MYSQL_TYPE_STRING, 0, 0, 0, SKIP_OPEN_TABLE},
  {"STEP_SIZE", NAME_LEN, MYSQL_TYPE_STRING, 0, 0, 0, SKIP_OPEN_TABLE},
  {"VALUE_BIN1", MY_INT64_NUM_DECIMAL_DIGITS, MYSQL_TYPE_LONGLONG, 0, 0, 0,
   SKIP_OPEN_TABLE},
  {"VALUE_BIN2", MY_INT64_NUM_DECIMAL_DIGITS, MYSQL_TYPE_LONGLONG, 0, 0, 0,
   SKIP_OPEN_TABLE},
  {"VALUE_BIN3", MY_INT64_NUM_DECIMAL_DIGITS, MYSQL_TYPE_LONGLONG, 0, 0, 0,
   SKIP_OPEN_TABLE},
  {"VALUE_BIN4", MY_INT64_NUM_DECIMAL_DIGITS, MYSQL_TYPE_LONGLONG, 0, 0, 0,
   SKIP_OPEN_TABLE},
  {"VALUE_BIN5", MY_INT64_NUM_DECIMAL_DIGITS, MYSQL_TYPE_LONGLONG, 0, 0, 0,
   SKIP_OPEN_TABLE},
  {"VALUE_BIN6", MY_INT64_NUM_DECIMAL_DIGITS, MYSQL_TYPE_LONGLONG, 0, 0, 0,
   SKIP_OPEN_TABLE},
  {"VALUE_BIN7", MY_INT64_NUM_DECIMAL_DIGITS, MYSQL_TYPE_LONGLONG, 0, 0, 0,
   SKIP_OPEN_TABLE},
  {"VALUE_BIN8", MY_INT64_NUM_DECIMAL_DIGITS, MYSQL_TYPE_LONGLONG, 0, 0, 0,
   SKIP_OPEN_TABLE},
  {"VALUE_BIN9", MY_INT64_NUM_DECIMAL_DIGITS, MYSQL_TYPE_LONGLONG, 0, 0, 0,
   SKIP_OPEN_TABLE},
  {"VALUE_BIN10", MY_INT64_NUM_DECIMAL_DIGITS, MYSQL_TYPE_LONGLONG, 0, 0, 0,
   SKIP_OPEN_TABLE},
  {"VALUE_BIN11", MY_INT64_NUM_DECIMAL_DIGITS, MYSQL_TYPE_LONGLONG, 0, 0, 0,
   SKIP_OPEN_TABLE},
  {"VALUE_BIN12", MY_INT64_NUM_DECIMAL_DIGITS, MYSQL_TYPE_LONGLONG, 0, 0, 0,
   SKIP_OPEN_TABLE},
  {"VALUE_BIN13", MY_INT64_NUM_DECIMAL_DIGITS, MYSQL_TYPE_LONGLONG, 0, 0, 0,
   SKIP_OPEN_TABLE},
  {"VALUE_BIN14", MY_INT64_NUM_DECIMAL_DIGITS, MYSQL_TYPE_LONGLONG, 0, 0, 0,
   SKIP_OPEN_TABLE},
  {"VALUE_BIN15", MY_INT64_NUM_DECIMAL_DIGITS, MYSQL_TYPE_LONGLONG, 0, 0, 0,
   SKIP_OPEN_TABLE},
  {0, 0, MYSQL_TYPE_STRING, 0, 0, 0, SKIP_OPEN_TABLE}
};
...
ST_FIELD_INFO processlist_fields_info[]=
{
  ...
  {"DB", NAME_CHAR_LEN, MYSQL_TYPE_STRING, 0, 1, "Db", SKIP_OPEN_TABLE},
  {"COMMAND", 16, MYSQL_TYPE_STRING, 0, 0, "Command", SKIP_OPEN_TABLE},
  {"TIME", MAX_DOUBLE_STR_LENGTH, MYSQL_TYPE_DOUBLE, 0, MY_I_S_UNSIGNED,
...
ST_SCHEMA_TABLE schema_tables[]=
{
...
#ifndef EMBEDDED_LIBRARY
  {"USER_LATENCY_HISTOGRAMS", user_latency_histograms_fields_info,
   create_schema_table, fill_user_histograms, NULL, NULL, -1, -1, false, 0},
#endif
  ...
  {"ADMISSION_CONTROL_ENTITIES", admission_control_entities_fields_info, create_schema_table,
   fill_ac_entities, NULL, NULL, -1, -1, false, 0},

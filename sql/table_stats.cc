                        ...
                        TABLE *tbl)
{
  ...
  {
    /*
      Keep things in sync after create or drop index. This doesn't notice
      create followed by drop. "reset statistics" will fix that.
    */
...
int fill_table_stats(THD *thd, TABLE_LIST *tables, Item *cond)
{
  ...
  for (unsigned i = 0; i < global_table_stats.records; ++i) {
    ...
    table->field[f++]->store(table_stats->io_perf_read_blob.wait_time_max, TRUE);
...
//      table->field[f++]->store(index_stats->io_perf_read.svc_time, TRUE);
//      table->field[f++]->store(index_stats->io_perf_read.svc_time_max, TRUE);
//      table->field[f++]->store(index_stats->io_perf_read.wait_time, TRUE);
//      table->field[f++]->store(index_stats->io_perf_read.wait_time_max, TRUE);
...
//      table->field[f++]->store(index_stats->io_perf_read_blob.svc_time, TRUE);
//      table->field[f++]->store(index_stats->io_perf_read_blob.svc_time_max, TRUE);
//      table->field[f++]->store(index_stats->io_perf_read_blob.wait_time, TRUE);
//      table->field[f++]->store(index_stats->io_perf_read_blob.wait_time_max, TRUE);

  ...
  if (hton->state == SHOW_OPTION_YES && hton->recover) {
    ...
    while (
      ...
      for (int i = 0; i < got; i++) {
        ...
        my_xid xid_val = xid->get_my_xid();
        std::string xid_str = std::to_string(xid_val);
        std::string gtid_str;

        if (info->commit_list) {
          const auto itr = info->commit_list->find(xid_val);
        ...
        if (!xid_val)  // not "mine" - that is generated by external TM
        {
          // NO_LINT_DEBUG
          sql_print_information(
        ...
        if (info->commit_list != nullptr) {
          auto it = info->commit_list->find(xid_val);
        ...
        if (recovery_mode_condition && !opt_trim_binlog &&
          ...
          // NO_LINT_DEBUG
          sql_print_information(
              "Committing XID: %s (GTID: %s)", xid_str.c_str(),
              gtid_str.empty() ? "Not found" : gtid_str.c_str());
          hton->commit_by_xid(hton, &info->list[i].id);
        } else {
          // NO_LINT_DEBUG
          sql_print_information(
              "Rolling back XID: %s (GTID: %s)", xid_str.c_str(),
              gtid_str.empty() ? "Not found" : gtid_str.c_str());

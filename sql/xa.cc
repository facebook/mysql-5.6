...
static bool sync_binlog_pos(THD *, plugin_ref plugin, void *arg) {
  ...
  */
  bool status = hton->sync_binlog_pos(hton, info->binlog_file, info->binlog_pos,
...
int ha_sync_binlog_pos(const char *binlog_file, my_off_t binlog_pos,
   ...
   */
  if (plugin_foreach(nullptr, sync_binlog_pos, MYSQL_STORAGE_ENGINE_PLUGIN,
                     &info)) {
    DBUG_RETURN(1);
  }
...
static void recover_binlog_pos(const char *plugin_name, handlerton *hton,
  ...
  if (max_gtid.greater_than(*info->binlog_max_gtid)) {
    *(info->binlog_max_gtid) = max_gtid;
    ...
    DBUG_ASSERT(is_binlog_advanced(info->binlog_file, *info->binlog_pos,

...
{
  DBUG_ASSERT(!enabled && !thd && !sysvars_ptr && t);
  thd = t;
  char *var = t->variables.track_sysvars_ptr;
  if (var)
  {
    sysvars_ptr = (char *) my_memdup(var, strlen(var) + 1, MYF(MY_WME));
    /* Update the variable to point to the newly alloced copy. */
    thd->variables.track_sysvars_ptr= sysvars_ptr;
  }
}
...
bool Session_sysvar_resource_manager::enable()
{
  ...
  if (sysvars_ptr)
  {
    ...
    if ( !(element=
      ...
      return true;                            /* Error */
    DBUG_ASSERT(thd->variables.track_sysvars_ptr == sysvars_ptr);
    element->data= (void *) sysvars_ptr;
    /* Reset this pointer after the memory ownership was transferred */
    sysvars_ptr = NULL;

...
int set_var_collation_client::update(THD *thd)
{
  ...
  if (thd->session_tracker.get_tracker(SESSION_SYSVARS_TRACKER)->is_enabled())
  {
    ...
    thd->session_tracker
      .get_tracker(SESSION_SYSVARS_TRACKER)->mark_as_changed(&cs_client);
    LEX_CSTRING cs_results= {"character_set_results",
                             sizeof("character_set_results") -1};
    thd->session_tracker
      .get_tracker(SESSION_SYSVARS_TRACKER)->mark_as_changed(&cs_results);
    LEX_CSTRING cs_connection= {"character_set_connection",
                                sizeof("character_set_connection") - 1};
    thd->session_tracker
      .get_tracker(SESSION_SYSVARS_TRACKER)->mark_as_changed(&cs_connection);
  }

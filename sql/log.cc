...
bool Log_to_csv_event_handler::log_gap_lock(
    THD *, ulonglong, const char *, size_t, my_thread_id,
...
bool Log_to_file_event_handler::log_gap_lock(
  ...
  thd->push_internal_handler(&error_handler);
  bool retval =
      mysql_gap_lock_log.write_general(event_utime, thread_id, command_type,
                                      command_type_len, sql_text, sql_text_len);
  thd->pop_internal_handler();
  return retval;
}
...
bool Query_logger::gap_lock_log_write(THD *thd, enum enum_server_command command,
                                      const char *query, size_t query_length)
{
  /*
  ...
  for (Log_event_handler **current_handler = gap_lock_log_handler_list;
    ...
    error |=
      (*current_handler++)
      ->log_gap_lock(thd, current_utime, user_host_buff, user_host_len,
          thd->thread_id(), command_name[(uint)command].str,

...
static bool explicit_snapshot_handlerton(THD *thd, 
                                         plugin_ref plugin, 
                                         void *arg) {
  handlerton *hton = plugin_data<handlerton *>(plugin);
...
bool ha_explicit_snapshot(THD *thd,
  ...
  if (hton == nullptr) {
    ...
    if (hton->state == SHOW_OPTION_YES && hton->explicit_snapshot) {
      ...
      my_printf_error(ER_UNKNOWN_ERROR, "Explicit snapshots are not supported by this engine",
                      MYF(0));
      DBUG_RETURN(true);
    }
  }
...
static bool shared_snapshot_handlerton(THD *thd,
                                       plugin_ref plugin,
                                       void *arg) {
  handlerton *hton = plugin_data<handlerton *>(plugin);
  snapshot_context_st *ss_ctx = static_cast<snapshot_context_st*>(arg);

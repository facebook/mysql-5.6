...
int ha_commit_low(THD *thd, bool all, bool async, bool run_after_commit)
{
  ...
  */
  ...
  if (!error && enable_raft_plugin &&
      thd->m_force_raft_after_commit_hook && thd->rli_slave)
  ...
  if (hton->state == SHOW_OPTION_YES && hton->recover)
  {
                               ...
                               info->binlog_max_gtid)) > 0)
    {
      ...
      for (int i=0; i < got; i ++)
      {
        ...
        // rollback all prepared txns in the engine. If raft is enabled, then we
        // rollback all prepared txns
        if ((info->commit_list ?
            (xid_to_gtid= (XID_TO_GTID*)
             my_hash_search(info->commit_list, (uchar *)&x, sizeof(x))) != 0 :
            tc_heuristic_recover == TC_HEURISTIC_RECOVER_COMMIT) &&
            !opt_trim_binlog && !enable_raft_plugin)
        {

...
class Item_lineage_info_builder : public Select_lex_visitor {
 public:
  bool visit_union(SELECT_LEX_UNIT *) override { return false; }
  bool visit_query_block(SELECT_LEX *) override { return false; }
  /**
 ...
 * visit_* methods to allow itself to be accepted by SELECT_LEX_UNIT, SELECT_LEX
 * and Item.
 *
 ...
 * looking up m_unit_map by SELECT_LEX_UNION * pointer.
 */
class Column_lineage_info_builder : public Select_lex_visitor {
 public:
   ...
   *
   * @param[in] unit The SELECT_LEX_UNIT union unit
   *
   * @retval FALSE if successful. Otherwise, TRUE is returned.
   */
  bool visit_union(SELECT_LEX_UNIT *unit) override {
    DBUG_TRACE;
    ...
    for (SELECT_LEX *query_block = unit->first_select(); query_block;
         query_block = query_block->next_select()) {
   ...
   *
   * @param[in] query_block The SELECT_LEX query block
   *
   * @retval FALSE if successful. Otherwise, TRUE is returned.
   */
  bool visit_query_block(SELECT_LEX *query_block) override {
    DBUG_TRACE;
    ...
    // visit all the inner unit and add to parents
    for (SELECT_LEX_UNIT *unit = query_block->first_inner_unit(); unit;
         unit = unit->next_unit()) {
    ...
    //                perform JOIN
    const auto &unit = query_block->first_inner_unit();
    if (!unit) {
      // Leaf query block. Iterate through all the item and their positions in
      // TABLE_LIST. There might be multiple TABLE_LIST
      ...
      // TABLE_LIST as semi-join nest. See also SELECT_LEX::resolve_subquery and
      // SELECT_LEX::merge_derived
      TABLE_LIST *first_leaf_table = nullptr;
      if (query_block->table_list.elements > 0) {
        first_leaf_table = query_block->table_list.first->first_leaf_table();
      }
      for (TABLE_LIST *t = first_leaf_table; t; t = t->next_leaf) {
      ...
      DBUG_PRINT("column_lineage_info",
                 ("inner union units under a SELECT_LEX not supported"));
      return true;
    }
   ...
   *
   * @param[in] table_ref The TABLE_LIST table reference
   *
   * @retval FALSE if successful. Otherwise, TRUE is returned.
   */
  bool build_table_lineage_info(TABLE_LIST *table_ref) {
    DBUG_TRACE;
  ...
  bool build_item_lineage_info(Item_field *item_field,
    ...
    TABLE_LIST *table_ref = item_field->table_ref;
    if (!table_ref) {
      DBUG_PRINT("column_lineage_info",
                 ("table (TABLE_LIST) of item_field (%s) is null",
  ...
  std::unordered_map<SELECT_LEX_UNIT *, Union_column_lineage_info *> m_unit_map;
  std::unordered_map<SELECT_LEX *, Query_block_column_lineage_info *>
      m_query_block_map;
  std::unordered_map<TABLE_LIST *, Table_column_lineage_info *> m_table_ref_map;
};

...
public:
  ...
  // Note that admission_control_exit() broadcasts the signal.
...
/**
  Global class used to enforce per database admission control limits.
  Before executing a query, every thread calls admission_control_enter() to
  check the limits and increment running_queries counter. Threads go into
  waiting mode if necessary at this stage.

  After completing query execution, every thread calls admission_control_exit()
  to decrement running_queries counter and signal any waiting threads.
*/
class DB_AC {

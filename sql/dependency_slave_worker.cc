...
bool Dependency_slave_worker::execute_group() {
  ...
  if (unlikely(err)) {
    info_thd->get_stmt_da()->reset_diagnostics_area();
...
int Dependency_slave_worker::execute_event(
  ...
  DBUG_EXECUTE_IF("dbug.dep_wait_before_update_execution", {
    if (ev->raw_event()->get_type_code() == binary_log::UPDATE_ROWS_EVENT) {
      ...
      assert(!debug_sync_set_action(info_thd, STRING_WITH_LEN(act)));
    }
  };);
...
void Dependency_slave_worker::start() {
  assert(scheduler->dep_queue.empty());
  assert(scheduler->dep_key_lookup.empty());
  assert(scheduler->keys_accessed_by_group.empty());
  assert(scheduler->dbs_accessed_by_group.empty());

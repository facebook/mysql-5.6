...
final class FacebookBuildServer {
  ...
  public function startProjectBuilds($async, $diff_id=null) {
    ...
    if ($async !== true) {
      ...
      while ($going) {
        exec("wget -q -O - "
          ."http://$server/job/$project/$buildnum/consoleText",
          $output);
        ...
        foreach ($output as &$line) {
          if (preg_match($test_reg, $line, $matches)) {
            $test = "$matches[2]";
            $res = "$matches[1]";
            if ($res === 'fail' && !array_key_exists($test, $failures)) {
              $failures[$test] = TRUE;
              $console->writeOut('%s', "[ $res ] $test\n");
            }
          } elseif (preg_match('/\AFinished: (SUCCESS|FAILURE)\Z/',
                    $line, $matches)) {
            if ($matches[1] === "SUCCESS") {
              $console->writeOut(
                "Build Tests Completed (With %s Failures).\n",
                count($failures));
            } else {
              $console->writeOut(
                "Build Test System Failure!  Bad Thing!  Look into this!\n");
            }
            $going = FALSE;
            break;
          }

    ...
    if (read_all || bitmap_is_set(table->read_set, f->field_index())) {
      switch (f->field_index()) {
        ...
        case FO_CODE:
          ...
          break;
        case FO_LEVEL:
          set_field_varchar_utf8(f, curr_row.level().c_str(),
                                 curr_row.level().length());
          break;
        case FO_MESSAGE:
          set_field_varchar_utf8(f, curr_row.message().c_str(),
                                 curr_row.message().length());
          break;
        case FO_QUERY_TEXT:
          set_field_longtext_utf8(f, curr_row.query_text().c_str(),
        ...
        case FO_LAST_RECORDED:
          ...
          break;
        case FO_DB_NAME:
          set_field_varchar_utf8(f, curr_row.db_name().c_str(),

			...
			os_thread_create */
{
	...
	while (srv_shutdown_state == SRV_SHUTDOWN_NONE) {
		...
		ib_int64_t      sig_count = os_event_reset(buf_lru_event);
		os_event_wait_time_low(buf_lru_event, 100000, sig_count);
...
/*======================================*/
{
	...
	if (srv_force_recovery < SRV_FORCE_NO_LOG_REDO) {
		...
		/* if the global var srv_recv_skip_ibuf_operations is set, we
		disallow ibuf operations during the apply log phase. */
		if (srv_recv_skip_ibuf_operations && !srv_read_only_mode) {
			mutex_enter(&(log_sys->mutex));
			recv_apply_hashed_log_recs(FALSE);
			mutex_exit(&(log_sys->mutex));
		} else {

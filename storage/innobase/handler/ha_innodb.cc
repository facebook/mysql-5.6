    ...
    trx_t *trx) /*!< in/out: InnoDB transaction handle */
{
  ...
  if (!thd->is_persist_variables_init()) {
    trx_lra_reset(trx, THDVAR(thd, lra_size),
                  THDVAR(thd, lra_pages_before_sleep), THDVAR(thd, lra_sleep),
                  THDVAR(thd, lra_n_spaces), true);
  }

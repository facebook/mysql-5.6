	...
	ulint		flags)	/*!< in: table flags */
{
	...
	case DB_LOCK_WAIT_TIMEOUT:
		...
		if (!db)
		{
			snprintf(msg, sizeof(msg), "Timeout on alter table: %.128s",
				 table);
		} else {
			snprintf(msg, sizeof(msg), "Timeout on alter table: %.64s.%.64s",
				 db, table);
		}
	...
	bool			new_clustered)
{
	...
	if (error != DB_SUCCESS) {
err_exit:
		my_error_innodb(error, db_name, user_table->name, 0);
		trx->error_state = DB_SUCCESS;
		trx->op_info = "";
		DBUG_RETURN(true);
	}

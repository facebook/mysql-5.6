...
/*==============*/
{
	...
	for(; i < srv_sync_pool_size; ++i) {
		...
		os_fast_mutex_init(event_os_mutex_key, &os_support[i].os_mutex);
#else
		os_fast_mutex_init(PFS_NOT_INSTRUMENTED, &os_support[i].os_mutex);
#endif
...
#ifndef PFS_SKIP_EVENT_MUTEX
	os_fast_mutex_init(event_os_mutex_key, &(event->ev.sup->os_mutex));
#else
	os_fast_mutex_init(PFS_NOT_INSTRUMENTED, &(event->ev.sup->os_mutex));
#endif

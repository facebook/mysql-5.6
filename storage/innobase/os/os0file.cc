...
struct os_aio_array_t{
	...
	ulint*			count;
				...
				counts the number of not-submitted aio request
				on that segment.*/
...
latency_histogram histogram_async_read;
latency_histogram histogram_async_write;

latency_histogram histogram_sync_read;
latency_histogram histogram_sync_write;
...
UNIV_INTERN volatile ulint	os_aio_n_outstanding = 0;
#ifdef UNIV_DEBUG
...
#if defined(XTRABACKUP) || defined(__WIN__)
/* InnoDB Hot Backup does not lock the data files.
 * On Windows, mandatory locking is used.
 */
# undef USE_FILE_LOCK
#endif /* XTRABACKUP || __WIN__ */
#ifdef USE_FILE_LOCK
/****************************************************************//**
...
#else /* __WIN__ */
	int		create_flag;
	const char*	mode_str	MY_ATTRIBUTE((unused)) = NULL;
...
retry:
	...
	if ((ulint) ret == n) {
		if (log_sys) {
			log_update_padding_info(padding, n, logical);
		}
		return(TRUE);
	}
...
skip_native_aio:
#endif /* LINUX_NATIVE_AIO */
	os_aio_slot_t* slot = os_aio_array_get_nth_slot(array, 0);
	for (uint i = 0; i < n; i++, slot++) {
		slot->pos = i;
		slot->reserved = FALSE;
#ifdef WIN_ASYNC_IO
				...
				array */
{
	...
	latency_histogram_init(&histogram_async_read,
		       innobase_histogram_step_size_async_read);
	latency_histogram_init(&histogram_async_write,
		       innobase_histogram_step_size_async_write);

	latency_histogram_init(&histogram_sync_read,
		       innobase_histogram_step_size_sync_read);
	latency_histogram_init(&histogram_sync_write,
		       innobase_histogram_step_size_sync_write);

	latency_histogram_init(&histogram_log_write,
	...
	os_aio_outstanding_requests_wait_event = os_event_create();
...
/*=============*/
{
	...
	os_event_free(os_aio_outstanding_requests_wait_event);
...
loop:
	...
	guaranteed to find a slot in full scan. */
        i = (local_seg * slots_per_seg) % array->n_slots;
	slot = os_aio_array_get_nth_slot(array, i);
	for (counter = 0;
	     counter < array->n_slots; i++, counter++, slot++) {
		if (i == array->n_slots) {
			i = 0;
				...
				arrays */
{
	...
	os_aio_slot_t* slot = os_aio_array_get_nth_slot(array, segment);
	for (uint i = 0; i < n; i++, slot++) {
...
os_aio_linux_dispatch_read_array_submit()
{
	...
	for (ulint i = 0; i < array->n_segments; i++) {
		...
		/* Update oustanding requests statistics. */
#if defined(HAVE_ATOMIC_BUILTINS) && UNIV_WORD_SIZE == 8
...
#ifdef UNIV_DEBUG
		/* Update max oustanding requests statistics.*/
...
/*==================*/
	...
	ibool		should_buffer)	/*!< in: should buffer the request
					rather than submit. */
{
	int		ret;
	ulint		io_ctx_index = 0;
	struct iocb*	iocb;
	ulint slots_per_segment;
...

{
	...
	if (mode == OS_AIO_SYNC
		...
		if (type == OS_FILE_READ) {
			...
			latency_histogram *current_histogram;
...
restart:
	...
	oldest_offset = lowest_offset = IB_UINT64_MAX;
	...
	if (oldest_request) {
		...
		if (biggest_age >= SRV_IO_OLD_TIME) {
			if (slot && slot->type == OS_FILE_WRITE)
	...
	slot_start = os_aio_array_get_nth_slot(array, segment * n);
consecutive_loop:
	slot = slot_start;
	for (uint i = 0; i < n; i++, slot++) {
		if (slot->reserved
	...
	os_aio_array_t*	array)	/*!< in: aio array to print */
{
	...
	os_aio_slot_t* slot = os_aio_array_get_nth_slot(array, 0);
	for (uint i = 0; i < array->n_slots; i++, slot++) {
		ulint	seg_no;

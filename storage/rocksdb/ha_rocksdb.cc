...
public:
  ...
  int set_status_error(THD *thd, rocksdb::Status &s, RDBSE_KEYDEF *key_descr,
...
bool ha_rocksdb::get_error_message(int error, String *buf)
{
  ...
  {
    buf->append(trx->detailed_error.c_str());
    return TRUE;
  }
    ...
    else
    {
      ...
      */
      ...
      if (!s.ok() && !s.IsNotFound())
        DBUG_RETURN(trx->set_status_error(table->in_use, s, key_descr[i], tbl_def));

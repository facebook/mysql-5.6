...
*/
static void update_row_counters(const TABLE* table, operation_type type) {
  ...
  if (scan_it->Valid())
  {
    ...
    if (key_descr[keyno]->covers_key(key.data(), key.size()))
    {
      ...
      if (!rc)
      {
        ...
        update_row_counters(table, ROWS_READ);
        return rc;
      }
    }
  }
...
int ha_rocksdb::write_row(uchar *buf)
{
  ...
  */
  ...
  {
    stats.rows_inserted++;
    update_row_counters(table, ROWS_INSERTED);
  }
  io_perf_end_and_record();
  DBUG_RETURN(rv);
}
...
int ha_rocksdb::delete_row(const uchar *buf)
{
  ...
  stats.rows_deleted++;
  update_row_counters(table, ROWS_DELETED);
...
int ha_rocksdb::update_row(const uchar *old_data, uchar *new_data)
{
  ...
  */
  ...
  {
    stats.rows_updated++;
    update_row_counters(table, ROWS_UPDATED);
  }
  DBUG_RETURN(rv);
}

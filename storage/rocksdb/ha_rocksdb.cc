                                    ...
                                    const key_range *end_key)
{
  ...
  */
  ...
  bool snapshot_existed_prior= (trx->has_snapshot() == false);
  ...
  for (;;)
  {
    ...
    */
    ...
    if (rc == HA_ERR_LOCK_DEADLOCK && snapshot_existed_prior)
    {
...
int ha_rocksdb::index_first_intern(uchar *buf)
{
  ...
  Rdb_transaction *trx= get_or_create_trx(table->in_use);
  bool snapshot_existed_prior= !trx->has_snapshot();
  // Loop as long as we get a deadlock error AND we end up creating the
  // snapshot here (i.e. it did not exist prior to this)
  for (;;)
  {
...
int ha_rocksdb::rnd_init(bool scan)
{
  ...
  if (scan)
  {
    rnd_scan_snapshot_existed_prior= !trx->has_snapshot();
...
int ha_rocksdb::rnd_next(uchar *buf)
{
  ...
  for (;;)
  {
    ...
    if (rc == HA_ERR_LOCK_DEADLOCK && rnd_scan_snapshot_existed_prior)
    {
  ...
  rnd_scan_snapshot_existed_prior= false;

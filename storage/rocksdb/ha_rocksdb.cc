                                      ...
                                      char               delimiter)
{
  ...
  for (pos = 0; pos < input.size(); pos++)
  {
    ...
    {
      // If we have found a delimiter and we have data, add it into the list.
      if (pos != beg_pos)
        elems.push_back(input.substr(beg_pos, pos - beg_pos));

      // Reset our beginning position to the next character
      beg_pos = pos + 1;
    }
  }

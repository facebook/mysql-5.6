...
static MYSQL_THDVAR_BOOL(commit_in_the_middle, PLUGIN_VAR_RQCMDARG,
  "Commit rows implicitly every rdb_bulk_load_size, on bulk load/insert",
  nullptr, nullptr, FALSE);
...
static MYSQL_SYSVAR_ULONGLONG(rate_limiter_bytes_per_sec,
  rdb_rate_limiter_bps_sysvar,
  PLUGIN_VAR_RQCMDARG,
  "DBOptions::rdb_rate_limiter bytes_per_sec for RocksDB",
...
static MYSQL_SYSVAR_LONGLONG(
  ...
  PLUGIN_VAR_RQCMDARG,
  "Size of the window for counting compaction_sequential_deletes",
  nullptr, rdb_set_compaction_options_sysvar,
...
static MYSQL_SYSVAR_BOOL(compaction_sequential_deletes_count_sd,
  ...
  PLUGIN_VAR_RQCMDARG,
  "Counting SingleDelete as compaction_sequential_deletes",
...
public:
  ...
  */
                                  ...
                                  bool create_snapshot= true)
  {
    ...
    rocksdb::Iterator* rdb_it= m_rocksdb_tx->GetIterator(options,
                                                         column_family);
    return rdb_it;
  }
...
void ha_rocksdb::load_auto_incr_value()
{
  ...
  handler::active_index= table->s->next_number_index;
                                            ...
                                            bool move_forward)
{
  ...
  if (m_key_is_read_only && !m_lock_rows && !rdb_table_has_hidden_pk(table))
  {
...
int ha_rocksdb::pk_index(const my_core::TABLE* table_arg,
                         const Rdb_tbl_def* tbl_def_arg)
{
  DBUG_ASSERT(table_arg != nullptr);
  DBUG_ASSERT(table_arg->s != nullptr);
...
int ha_rocksdb::write_row(uchar *buf)
{
  ...
  DBUG_ASSERT(buf == handler::table->record[0]);
  DBUG_ASSERT(m_lock_rows && m_lock_for_write);
...
THR_LOCK_DATA **ha_rocksdb::store_lock(my_core::THD *thd,

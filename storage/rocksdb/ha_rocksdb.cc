...
static void* drop_index_thread(void*);
...
bool can_use_bloom_filter(THD *thd,
                          ...
                          bool is_ascending);
static Rdb_table_handler* rdb_get_open_table_handler(const char *table_name);
static int rdb_release_table_handler(Rdb_table_handler *table_handler);
...
*/
static my_core::HASH rdb_open_tables_hash;
/* The mutex used to init the hash; variable for example share methods */
mysql_mutex_t rdb_open_tables_mutex;
...
uchar* rdb_get_open_table_hash_key(
    ...
    const char* engine))
{
  ...
  for (auto it : tablenames)
  {
    ...
    table_handler= rdb_get_open_table_handler(it.c_str());
     ...
     */
     ...
     rdb_release_table_handler(table_handler);
...
static int rocksdb_init_func(void *p)
{
  ...
  mysql_mutex_init(rdb_psi_open_tbls_mutex_key, &rdb_open_tables_mutex,
                   MY_MUTEX_INIT_FAST);
  ...
  (void) my_hash_init(&rdb_open_tables_hash, my_core::system_charset_info,
                      32, 0, 0,
                      (my_hash_get_key) rdb_get_open_table_hash_key,
                      0, 0);
  ...
  if (db_options.allow_mmap_reads && !db_options.allow_os_buffer)
  {
    ...
    my_hash_free(&rdb_open_tables_hash);
    DBUG_RETURN(1);
  }
...
static int rocksdb_done_func(void *p)
{
  ...
  if (rdb_open_tables_hash.records)
  {
  ...
  mysql_mutex_destroy(&rdb_open_tables_mutex);
...
static Rdb_table_handler* rdb_get_open_table_handler(const char *table_name)
{
  ...
  mysql_mutex_lock(&rdb_open_tables_mutex);
  if (!(table_handler= reinterpret_cast<Rdb_table_handler*>(
          my_hash_search(&rdb_open_tables_hash,
                         ...
                         length))))
  {
                            ...
                            NullS))))
    {
      ...
      mysql_mutex_unlock(&rdb_open_tables_mutex);
      return nullptr;
    }
    ...
    if (my_hash_insert(&rdb_open_tables_hash,
...
std::vector<std::string> rdb_get_open_table_names(void)
{
  ...
  for (i = 0;
       (table_handler= reinterpret_cast<const Rdb_table_handler*>(
          my_hash_const_element(&rdb_open_tables_hash, i)));
       i++)
  {
    DBUG_ASSERT(table_handler != nullptr);
    names.push_back(table_handler->m_table_name);
  }
  DBUG_ASSERT(i == rdb_open_tables_hash.records);
  mysql_mutex_unlock(&rdb_open_tables_mutex);
...
static int rdb_release_table_handler(Rdb_table_handler *table_handler)
{
  ...
  if (!--table_handler->m_ref_count)
  {
    ...
    auto ret __attribute__((__unused__)) =
        my_hash_delete(&rdb_open_tables_hash,
                       reinterpret_cast<uchar*>(table_handler));
    DBUG_ASSERT(!ret);  // the hash entry must actually be found and deleted
    my_free(table_handler);
  }

  ref_count= table_handler->m_ref_count;
  mysql_mutex_unlock(&rdb_open_tables_mutex);
...
void ha_rocksdb::convert_record_to_storage_format(const uchar *pk_packed_tuple,
                                                  ...
                                                  rocksdb::Slice *packed_rec)
{
  ...
  {
    uint32_t key_crc32= my_core::crc32(0, pk_packed_tuple, pk_packed_size);
...
int ha_rocksdb::open(const char *name, int mode, uint test_if_locked)
{
  ...
  {
    rdb_release_table_handler(m_table_handler);
  }
  m_table_handler= rdb_get_open_table_handler(name);
  if (m_table_handler == nullptr)
  {
    DBUG_RETURN(HA_ERR_INTERNAL_ERROR);
  }
  ...
  */
  ...
  // TODO(?): move this into rdb_get_open_table_handler() ??
  m_pk_descr->setup(table, m_tbl_def);
  ...
  */
  ...
  uint max_packed_sec_key_len= pack_key_len;
...
int ha_rocksdb::close(void)
{
  ...
  DBUG_ASSERT(ret == 0);  // this must be the last reference held on the object
                                    ...
                                    const key_range *end_key)
{
  ...
  */
  ...
  rocksdb::Slice slice(
      reinterpret_cast<const char*>(m_sec_key_packed_tuple), packed_size);
                                 ...
                                 const bool skip_unique_check)
{
    ...
    if (old_data)
    {
      ...
      else
      {
        ...
        old_key_slice= rocksdb::Slice(
            reinterpret_cast<const char*>(m_sec_key_packed_tuple_old),
            old_packed_size);
      }
    }
  ...
  */
  for (uint i= 0; i < m_tbl_def->m_key_count; i++)
  {
    ...
    */
    ...
    new_key_slice= rocksdb::Slice(
        reinterpret_cast<const char*>(m_sec_key_packed_tuple), new_packed_size);
    new_value_slice= rocksdb::Slice(
        reinterpret_cast<const char*>(m_sec_key_tails), new_key_tail_size);
  ...
  convert_record_to_storage_format(m_pk_packed_tuple,
...
static void signal_drop_index_thread(bool stop_thread)
{
                                ...
                                Rdb_perf_counters *counters)
{
  ...
  Rdb_table_handler *table_handler;
  table_handler= rdb_get_open_table_handler(tablename);
  if (table_handler == nullptr)
  {
    return HA_ERR_INTERNAL_ERROR;
  }

...
static void register_snapshot(const rocksdb::Snapshot* snapshot, THD *thd)
{
  ...
  auto res __attribute__((__unused__)) = snapshot_map.emplace(snapshot,
...
static void register_snapshot_release(const rocksdb::Snapshot* snapshot)
{
  ...
  {
    mysql_mutex_lock(&snapshot_mutex);
    auto count __attribute__((__unused__)) = snapshot_map.erase(snapshot);
    mysql_mutex_unlock(&snapshot_mutex);
...
int ha_rocksdb::load_hidden_pk_value()
{
  ...
  if (!index_last(table->record[0]))
  {
    ...
    */
    ...
    hidden_pk_id += 1;
    if (tbl_def->hidden_pk_val < hidden_pk_id)
      tbl_def->hidden_pk_val= hidden_pk_id;
  }

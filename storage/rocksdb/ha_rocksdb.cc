...
bool ha_rocksdb::can_use_single_delete(uint index)
{
  ...
  */
  if (index != table->s->primary_key)
    return true;
  /*
     If the table has PRIMARY KEY only, and if all of the columns of the table
     are covered by the PRIMARY KEY, SingleDelete can be used.
  */
  if (table->s->keys == 1 && !strcmp(table->key_info[0].name, "PRIMARY") &&
      table->key_info[0].actual_key_parts == table->s->fields)
    return true;
    ...
    if (i == table->s->primary_key)
    {
      if (pk_key_changed)
      {
        ...
        if (can_use_single_delete(i))
          s= trx->SingleDelete(key_descr[i]->get_cf(),
                               old_pk_key_slice);
        else
          s= trx->Delete(key_descr[i]->get_cf(),
                         old_pk_key_slice);
        if (!s.ok())
        {
          DBUG_RETURN(return_status_error(table->in_use, s));
    ...
    */
    ...
    if (old_data)
    {
      ...
      */
      ...
      rocksdb::Status s;
      if (can_use_single_delete(i))
        s = trx->SingleDelete(key_descr[i]->get_cf(), old_key_slice);
      else
        s = trx->Delete(key_descr[i]->get_cf(), old_key_slice);
      if (!s.ok())
      {
        DBUG_RETURN(return_status_error(table->in_use, s));
      }
...
int ha_rocksdb::delete_row(const uchar *buf)
{
  ...
  Rdb_transaction *trx= get_or_create_trx(table->in_use);
  rocksdb::Status s;
  if (can_use_single_delete(table->s->primary_key))
    s= trx->SingleDelete(pk_descr->get_cf(), key_slice);
  else

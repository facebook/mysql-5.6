...
bool ha_rocksdb::get_se_private_data(dd::Table *dd_table, bool reset) {
  // TODO(laurynas): if/when MyRocks implements HTON_SUPPORTS_ATOMIC_DDL, look
  // into removing the ID assignment from this method, and moving it to
  // create_table, unified with user table ID assignment. The assigned IDs then
  // can be written to the dd::Table * argument, and will be persisten by the DD
  // layer if HTON_SUPPORTS_ATOMIC_DDL is set.

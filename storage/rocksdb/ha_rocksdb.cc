                          ...
                          bool is_ascending)
{
  ...
  const rocksdb::SliceTransform* rdb_prefix_extractor=
    rocksdb_cf_options_map.m_cf_to_prefix_extractor[kd.get_cf()->GetName()];

  if (rdb_prefix_extractor)
  {
    /*
    ...
    */
    if (use_all_keys && rdb_prefix_extractor->InRange(eq_cond))
      can_use= true;
    else if (!is_ascending)
      can_use= false;
    else if (rdb_prefix_extractor->SameResultWhenAppended(eq_cond))
      can_use= true;
    else
      can_use= false;
  } else

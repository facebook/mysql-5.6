 ...
 */
static size_t write_xid(XID *src, uchar *dst) {
  long f = src->formatID;
  long g = src->gtrid_length;
  long b = src->bqual_length;

  store_big_uint4(dst, *reinterpret_cast<uint64_t*>(&f));
  dst += 4;
  store_big_uint4(dst, *reinterpret_cast<uint64_t*>(&g));
  dst += 4;
  store_big_uint4(dst, *reinterpret_cast<uint64_t*>(&b));
  dst += 4;
 ...
 * in a string.
 */
static void read_xid(uchar *src, XID *dst)
{
  uint64_t f = read_big_uint4(src);
  src += 4;
  uint64_t g = read_big_uint4(src);
  src += 4;
  uint64_t b = read_big_uint4(src);
  src += 4;
                           ...
                           char* binlog_file, my_off_t* binlog_pos)
{
  ...
  for (auto it= trans_list.begin();
    ...
    read_xid((uchar*)(*it)->GetName().c_str(), xid);
    count++;
  }
  return count;
}
...
int ha_rocksdb::index_first_intern(uchar *buf)
{
  ...
  for (;;)
  {
    setup_scan_iterator(key_descr[active_index], &index_key, false,
                        !key_descr[active_index]->m_is_reverse_cf,
    ...
    */
    ...
    if (is_pk(i, table, tbl_def))
    {
      ...
      */
      ...
      if (!s.ok() && !s.IsNotFound())
        DBUG_RETURN(tx->set_status_error(table->in_use, s, key_descr[i],
                                         tbl_def));
    ...
    else
    {
      ...
      */
      ...
      rocksdb::Status s= tx->get_for_update(key_descr[i]->get_cf(),
                                            new_key_slice,
                                            &dummy_value);
      if (!s.ok() && !s.IsNotFound())
        DBUG_RETURN(tx->set_status_error(table->in_use, s, key_descr[i],
      ...
      rocksdb::Iterator *s_iter= tx->get_iterator(key_descr[i]->get_cf(),
                                                  total_order_seek,
                                                  fill_cache,

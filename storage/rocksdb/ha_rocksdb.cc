...
void ha_rocksdb::load_auto_incr_value() {
  ...
  if (!autoinc_upgrade_test &&
      dict_manager.get_auto_incr_val(m_tbl_def->get_pk()->get_gl_index_id(),
                                     &dd_val)) {
  ...
  if (dd_val == 0 || validate_last) {
    ...
    if (!index_last(table->record[0])) {
      ...
      if (validate_last && last_val <= max_val) {
        ...
        if (dict_manager.get_auto_incr_val(
                m_tbl_def->get_pk()->get_gl_index_id(), &dd_val)) {
...
void ha_rocksdb::update_auto_incr_val_from_field() {
  ...
  if (new_val <= max_val) {
    Rdb_transaction *const tx = get_or_create_tx(table->in_use);
    tx->set_auto_incr(m_tbl_def->get_pk()->get_gl_index_id(), new_val);
  ...
  if (create_info->auto_increment_value) {
    ...
    if (!autoinc_upgrade_test) {
      auto s = dict_manager.put_auto_incr_val(
          batch, m_tbl_def->get_pk()->get_gl_index_id(),

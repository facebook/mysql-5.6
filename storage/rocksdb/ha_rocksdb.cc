...
static int free_share(ROCKSDB_SHARE *share_arg);
...
/*
   ...
   methods
*/
static my_core::HASH rocksdb_open_tables;
...
public:
  ...
  int set_status_error(THD *thd, const rocksdb::Status &s,
                       Rdb_key_def *m_key_descr_arr, Rdb_tbl_def *m_tbl_def)
  {
    DBUG_ASSERT(!s.ok());
    ...
    if (s.IsTimedOut())
    {
      ...
      m_detailed_error.copy(timeout_message(
          "index", m_tbl_def->m_dbname_tablename.c_ptr(),
          m_key_descr_arr->get_name().c_str()));
...
{
  mysql_mutex_lock(&rocksdb_mutex);
  if (!--share_arg->use_count)
  {
    my_hash_delete(&rocksdb_open_tables, (uchar*) share_arg);
    thr_lock_delete(&share_arg->lock);
    //mysql_mutex_destroy(&share->mutex);
    my_free(share_arg);
  }
  mysql_mutex_unlock(&rocksdb_mutex);
...
ha_rocksdb::ha_rocksdb(handlerton *hton, TABLE_SHARE *table_arg)
  : handler(hton, table_arg), m_scan_it(nullptr),
    ...
    m_keyread_only(FALSE), m_field_enc_arr(nullptr),
...
/**
  ...
  @param m_pk_packed_tuple      Packed PK tuple. We need it in order to compute
                              and store its CRC.
  @param pk_packed_size       Length of the above
  @param packed_rec      OUT  Data slice with record data.
*/

void ha_rocksdb::convert_record_to_storage_format(const char *m_pk_packed_tuple,
                                                  ...
                                                  rocksdb::Slice *packed_rec)
{
  ...
  for (uint i=0; i < table->s->fields; i++)
  {
    ...
    if (m_field_enc_arr[i].m_dont_store)
      continue;

    Field *field= table->field[i];
    if (m_field_enc_arr[i].maybe_null())
    {
      char *data= (char*)m_storage_record.ptr();
      if (field->is_null())
      {
        data[m_field_enc_arr[i].m_null_offset]|= m_field_enc_arr[i].m_null_mask;
        /* Don't write anything for NULL values */
        continue;
      }
    }

    if (m_field_enc_arr[i].m_field_type == MYSQL_TYPE_BLOB)
    {
      ...
      m_storage_record.append((char*)blob->ptr, length_bytes);
    ...
    else if (m_field_enc_arr[i].m_field_type == MYSQL_TYPE_VARCHAR)
    {
      ...
      m_storage_record.append((char*)field_var->ptr,
    ...
    else
    {
      ...
      m_storage_record.append((char*)field->ptr, len);
    }
  }

  if (should_store_checksums())
  {
    uint32_t key_crc32= crc32(0, (uchar*)m_pk_packed_tuple, pk_packed_size);
    uint32_t val_crc32= crc32(0, (uchar*)m_storage_record.ptr(),
...
void ha_rocksdb::setup_read_decoders()
{
  ...
  for (uint i= 0; i < table->s->fields; i++)
  {
        ...
        bitmap_is_set(table->read_set, table->field[i]->field_index))
    {
      ...
      m_field_decoders.push_back({&m_field_enc_arr[i], true, skip_size});
      last_useful= m_field_decoders.size();
      skip_size= 0;
    }
    else
    {
      if (m_field_enc_arr[i].uses_variable_len_encoding())
      {
        // For variable-length field, we need to read the data and skip it
        m_field_decoders.push_back({&m_field_enc_arr[i], false, skip_size});
        skip_size= 0;
      }
      else
      {
        ...
        skip_size += m_field_enc_arr[i].m_pack_length_in_rec;
      }
    }
  }
  ...
  m_field_decoders.erase(m_field_decoders.begin() + last_useful,
                         m_field_decoders.end());
}
                                                   ...
                                                   uchar * buf)
{
  ...
  */
  ...
  for (auto it= m_field_decoders.begin(); it != m_field_decoders.end(); it++)
  {
...
void ha_rocksdb::setup_field_converters()
{
  ...
  DBUG_ASSERT(m_field_enc_arr == nullptr);
  m_field_enc_arr= static_cast<FIELD_ENCODER*>(
      my_malloc(table->s->fields * sizeof(FIELD_ENCODER), MYF(0)));
  if (m_field_enc_arr == nullptr)
  {
    return;
  }
  ...
  {
    Field *field= table->field[i];
    m_field_enc_arr[i].m_dont_store= false;
      ...
      for (uint kp= 0; kp < pk_info->user_defined_key_parts; kp++)
      {
        ...
        if (field->field_index + 1 == pk_info->key_part[kp].fieldnr)
        {
          if (m_pk_descr->can_unpack(kp))
            m_field_enc_arr[i].m_dont_store= true;
          break;
        }
      }
    }

    m_field_enc_arr[i].m_field_type= field->real_type();
    m_field_enc_arr[i].m_field_index= i;
    m_field_enc_arr[i].m_pack_length_in_rec= field->pack_length_in_rec();

    if (field->real_maybe_null())
    {
      m_field_enc_arr[i].m_null_mask= cur_null_mask;
      m_field_enc_arr[i].m_null_offset= null_bytes;
      ...
      m_field_enc_arr[i].m_null_mask= 0;
  }
...
int ha_rocksdb::open(const char *name, int mode, uint test_if_locked)
{
  ...
  m_share= get_share(name, table);
  if (m_share == nullptr)
    DBUG_RETURN(HA_ERR_INTERNAL_ERROR);

  my_core::thr_lock_data_init(&m_share->lock, &m_lock, nullptr);
  m_io_perf.init(&m_share->table_perf_context, &m_share->io_perf_read, &stats);
  ...
  */
  ...
  m_tbl_def= ddl_manager.find((uchar*)fullname.c_ptr(), fullname.length());
  ...
  m_pk_descr->setup(table, m_tbl_def);  // move this into get_share() ??

  uint packed_key_len= m_pk_descr->max_storage_fmt_length();

  if (!(m_pk_tuple= (uchar*)my_malloc(key_len, MYF(0))) ||
      !(m_pk_packed_tuple= (uchar*)my_malloc(packed_key_len, MYF(0))))
    DBUG_RETURN(HA_ERR_INTERNAL_ERROR);
...
int ha_rocksdb::close(void)
{
  ...
  my_free(m_field_enc_arr);
  m_field_enc_arr= nullptr;

  DBUG_RETURN(free_share(m_share));
}
                                ...
                                const uint len, HA_CREATE_INFO *create_info)
{
  ...
  */
  ...
  m_key_descr_arr= new Rdb_key_def*[n_keys];

  memset(m_key_descr_arr, 0, sizeof(Rdb_key_def*) * n_keys);
  ...
  */
  for (i= 0; i < m_tbl_def->m_key_count; i++)
  {
    ...
    m_key_descr_arr[i]= new Rdb_key_def(index_id, i,
                                   cf_handles[i],
                                   index_dict_version,
                                   index_type,
                                   kv_version,
  ...
  if (m_scan_it->Valid())
  {
    ...
    if (m_key_descr_arr[keyno]->covers_key(key))
    {
          ...
          !has_hidden_pk(table))
      {
        ...
        if (m_key_descr_arr[keyno]->unpack_record(this, table, buf, &key,
                                                  &value))
        {
          return HA_ERR_INTERNAL_ERROR;
        }
                                    ...
                                    const key_range *end_key)
{
  ...
  Rdb_key_def *kd= m_key_descr_arr[active_index];
  ...
  */
  ...
  rocksdb::Slice slice((char*)m_sec_key_packed_tuple, packed_size);
...
int ha_rocksdb::find_icp_matching_index_rec(bool move_forward, uchar *buf)
{
  ...
  if (pushed_idx_cond && pushed_idx_cond_keyno == active_index)
  {
    ...
    while (1)
    {
      ...
      {
        rocksdb::Slice prefix((const char*)m_sec_key_match_prefix,
...
int ha_rocksdb::index_first_intern(uchar *buf)
{
  ...
  if (m_key_descr_arr[active_index]->m_is_reverse_cf)
    m_key_descr_arr[active_index]->get_supremum_key(key, &key_size);
  else
    m_key_descr_arr[active_index]->get_infimum_key(key, &key_size);
  ...
  for (;;)
  {
    setup_scan_iterator(m_key_descr_arr[active_index], &index_key, false,
                        !m_key_descr_arr[active_index]->m_is_reverse_cf,
                                 ...
                                 const bool skip_unique_check)
{
  ...
  for (uint i= 0; i < m_tbl_def->m_key_count; i++)
  {
    ...
    */
    if (old_data)
    {
      ...
      {
        if (!m_update_scope.is_set(i))
          continue;

        old_packed_size= m_key_descr_arr[i]->pack_record(
    ...
    */
    ...
    else
    {
      ...
      */
      new_packed_size= m_key_descr_arr[i]->pack_record(
          this, table, m_pack_buffer, new_data, m_sec_key_packed_tuple,
          nullptr, nullptr, user_defined_key_parts, &n_null_fields, 0);
    ...
    */
    ...
    if (is_pk(i, table, m_tbl_def))
    {
      ...
      */
      ...
      if (!s.ok() && !s.IsNotFound())
        DBUG_RETURN(tx->set_status_error(table->in_use, s, m_key_descr_arr[i],
                                         m_tbl_def));
    ...
    else
    {
      bool all_parts_used= (user_defined_key_parts ==
                            m_key_descr_arr[i]->get_key_parts());
      /*
      ...
      */
      bool total_order_seek= !can_use_bloom_filter(
          ha_thd(), m_key_descr_arr[i], new_key_slice, all_parts_used,
          is_ascending(m_key_descr_arr[i], HA_READ_KEY_EXACT));
      ...
      */
      ...
      rocksdb::Status s= tx->get_for_update(m_key_descr_arr[i]->get_cf(),
                                            new_key_slice,
                                            &dummy_value);
      if (!s.ok() && !s.IsNotFound())
        DBUG_RETURN(tx->set_status_error(table->in_use, s, m_key_descr_arr[i],
      ...
      rocksdb::Iterator *s_iter= tx->get_iterator(m_key_descr_arr[i]->get_cf(),
                                                  total_order_seek,
                                                  fill_cache,
      ...
      */
      ...
      found= !read_key_exact(m_key_descr_arr[i], s_iter, all_parts_used,
                             new_key_slice);
      delete s_iter;
    }
  ...
  */
  for (uint i= 0; i < m_tbl_def->m_key_count; i++)
  {
    ...
    */
    if (is_pk(i, table, m_tbl_def))
    {
      if (pk_key_changed)
      {
        rocksdb::Status s= delete_or_singledelete(i, tx,
    ...
    */
    if (old_data && !m_update_scope.is_set(i))
      continue;

    new_packed_size= m_key_descr_arr[i]->pack_record(this, table, m_pack_buffer,
                                               new_data,
    ...
    if (old_data)
    {
      ...
      old_packed_size= m_key_descr_arr[i]->pack_record(
          this, table, m_pack_buffer, old_data, m_sec_key_packed_tuple_old,
          m_sec_key_tails_old, &old_key_tail_size, 0, nullptr, hidden_pk_id);
      old_key_slice= rocksdb::Slice((const char*)m_sec_key_packed_tuple_old,
                                    old_packed_size);
      ...
      */
      ...
      /*
        ...
        (also note that DDL statements do not delete rows, so this is not a DDL
         statement)
      */
      tx->get_indexed_write_batch()->SingleDelete(m_key_descr_arr[i]->get_cf(),
                                                  old_key_slice);
    }
    ...
    */
    ...
    write_batch->Put(m_key_descr_arr[i]->get_cf(), new_key_slice,
                     new_value_slice);
  }
  ...
  convert_record_to_storage_format((const char*)m_pk_packed_tuple,
...
int ha_rocksdb::delete_row(const uchar *buf)
{
  ...
  for (uint i= 0; i < m_tbl_def->m_key_count; i++)
  {
    ...
    {
      int packed_size;
      packed_size= m_key_descr_arr[i]->pack_record(this, table, m_pack_buffer,
                                             buf, m_sec_key_packed_tuple,
                                             nullptr, nullptr, 0, nullptr,
                                             hidden_pk_id);
      rocksdb::Slice secondary_key_slice((char*)m_sec_key_packed_tuple,
                                         packed_size);
...
void ha_rocksdb::calc_updated_indexes()
{
  if (!m_update_scope_is_valid)
  {
    ...
    for (uint keynr= 0; keynr < table->s->keys; keynr++)
    {
      ...
      uint key_parts= m_key_descr_arr[keynr]->get_key_parts();
      for (uint kp= 0; kp < key_parts; kp++)
      {
        ...
        Field *field=
            m_key_descr_arr[keynr]->get_table_field_for_part_no(table, kp);
                                     ...
                                     key_range *max_key)
{
  ...
  Rdb_key_def *kd= m_key_descr_arr[inx];
  DBUG_ASSERT(kd);
...
int ha_rocksdb::analyze(THD* thd, HA_CHECK_OPT* check_opt)
{
  ...
  for (uint i = 0; i < table->s->keys; i++)
  {
    ...
    ranges[m_key_descr_arr[i]->get_cf()].push_back(get_range(i, bufp));
    ids_to_check.insert(m_key_descr_arr[i]->get_gl_index_id());
  }
    ...
    my_core::Alter_inplace_info *ha_alter_info)
{
  ...
  Rdb_key_def** old_key_descr= m_tbl_def->m_key_descr;
                                ...
                                Rdb_perf_counters *counters)
{
  ...
  ROCKSDB_SHARE *tmp_share= get_share(tablename, nullptr);
  if (tmp_share == nullptr)
    return HA_ERR_INTERNAL_ERROR;

  counters->load(tmp_share->table_perf_context);

...
class Rdb_perf_context
{
 public:
  std::atomic_ullong m_value[PC_MAX_IDX];

  void dump_out(Rdb_perf_counters *counters) const;
};
...
class Rdb_perf_counters
{
 public:
  ...
  void start();
  void end_and_record(Rdb_perf_context *table_perf_context);

  void dump_global_counters() {
    s_global_perf_context.dump_out(this);
  }

 private:
  static Rdb_perf_context s_global_perf_context;

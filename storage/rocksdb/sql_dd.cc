...
void dd_add_instant_columns(const TABLE *old_table, const TABLE *altered_table,
  ...
  DBUG_ASSERT(altered_table->s->fields > old_table->s->fields);

#ifdef UNIV_DEBUG
  for (uint32_t i = 0; i < old_table->s->fields; ++i) {
    DBUG_ASSERT(strcmp(old_table->field[i]->field_name,
  ...
  for (uint32_t i = old_table->s->fields; i < altered_table->s->fields; ++i) {
    ...
    DBUG_ASSERT(field_type != MYSQL_TYPE_BLOB &&
                field_type != MYSQL_TYPE_JSON && field_type != MYSQL_TYPE_TEXT);
#endif
...
#ifdef UNIV_DEBUG
  DBUG_ASSERT(num_instant_cols > 0);
#endif /* UNIV_DEBUG */
}
...
void dd_copy_table_columns(dd::Table &new_table, const dd::Table &old_table) {
  ...
  for (const auto old_col : old_table.columns()) {
    ...
    if (new_col == nullptr) {
      DBUG_ASSERT(old_col->is_virtual());
      continue;
    }
...
uchar *DD_instant_col_val_coder::decode(const char *stream, size_t in_len,
  ...
  for (size_t i = 0; i < in_len / 2; ++i) {
    ...
    DBUG_ASSERT(isdigit(c1) || (c1 >= 'a' && c1 <= 'f'));
    DBUG_ASSERT(isdigit(c2) || (c2 >= 'a' && c2 <= 'f'));

...
#ifdef UNIV_DEBUG
  DBUG_ASSERT(!instant || dd_instant_columns_exist(table));
#endif /* UNIV_DEBUG */
...
inline void dd_commit_instant_table(
  ...
  DBUG_ASSERT(!new_dd_tab->is_temporary());
  DBUG_ASSERT(old_dd_tab->columns().size() <= new_dd_tab->columns()->size());
  ...
  DBUG_ASSERT(dd_table_has_instant_cols(*new_dd_tab));
}
}  // namespace myrocks

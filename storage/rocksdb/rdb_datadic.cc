...
{
  static const uchar* SLAVE_GTID_INFO_TABLE=
    (const uchar*)"mysql.slave_gtid_info";
  ...
  if (id && db && gtid) {
    ...
    if (!m_slave_gtid_info_tbl.load()) {
      m_slave_gtid_info_tbl.store(
        get_ddl_manager()->find(SLAVE_GTID_INFO_TABLE,
                                UT_ARRAY_SIZE(SLAVE_GTID_INFO_TABLE)));
    }
    if (!m_slave_gtid_info_tbl.load()) {

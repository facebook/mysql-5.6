...
Rdb_perf_context Rdb_perf_counters::s_global_perf_context;

void Rdb_perf_counters::end_and_record(Rdb_perf_context *table_perf_context)
{
  harvest_diffs();
  ...
  for (int i= 0; i < PC_MAX_IDX; i++) {
    if (m_value[i]) {
      if (table_perf_context) {
        table_perf_context->m_value[i] += m_value[i];
      }
      s_global_perf_context.m_value[i] += m_value[i];
    }
  }
}

void Rdb_perf_context::dump_out(Rdb_perf_counters *counters) const
{
  DBUG_ASSERT(counters != nullptr);

  for (int i= 0; i < PC_MAX_IDX; i++) {
    counters->m_value[i]= m_value[i].load(std::memory_order_relaxed);
  }
}
...
void Rdb_io_perf::end_and_record(uint32_t perf_context_level)
{
  ...
  m_local_perf_context.end_and_record(m_table_perf_context);

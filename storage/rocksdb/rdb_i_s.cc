...
static int store_global_info(THD *thd,
                             TABLE_LIST *tables,
                             const char *type,
                             const char *name,
                             const char *value)
                                            ...
                                            Item *cond)
{
  ...
  if (blm->read(file_buf, pos, gtid_buf)) {
    ...
    ret |= store_global_info(thd, tables, "BINLOG", "FILE", file_buf);
    ret |= store_global_info(thd, tables, "BINLOG", "POS", pos_buf);
    ret |= store_global_info(thd, tables, "BINLOG", "GTID", gtid_buf);
  }

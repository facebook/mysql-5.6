...
static int rdb_i_s_global_info_fill_table(
  ...
  if (blm->read(&marker)) {
    snprintf(pos_buf, INT_BUF_LEN, "%" PRIu64, (uint64_t)marker.offset);

    ret |= rdb_global_info_fill_row(thd, tables, "BINLOG", "FILE", marker.file);
    ret |= rdb_global_info_fill_row(thd, tables, "BINLOG", "POS", pos_buf);
    ret |= rdb_global_info_fill_row(thd, tables, "BINLOG", "GTID",
                                    marker.max_gtid);
  }

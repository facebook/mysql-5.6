...
/*
  Support for INFORMATION_SCHEMA.ROCKSDB_GENERIC_INFO dynamic table
 */
static int i_s_rocksdb_generic_info_fill_table(THD *thd,
                                            TABLE_LIST *tables,
                                            Item *cond)
{
  DBUG_ENTER("i_s_rocksdb_generic_info_fill_table");
  ...
  if (blm->read(file_buf, pos, gtid_buf)) {
    snprintf(pos_buf, FN_REFLEN, "%llu", pos);

    // binlog file
    tables->table->field[0]->store(STRING_WITH_LEN("BINLOG"),
        system_charset_info);
    tables->table->field[1]->store(STRING_WITH_LEN("FILE"),
        system_charset_info);
    tables->table->field[2]->store(file_buf, strlen(file_buf),
        system_charset_info);
    ret = schema_table_store_record(thd, tables->table);
    ...
    tables->table->field[0]->store(STRING_WITH_LEN("BINLOG"),
        system_charset_info);
    tables->table->field[1]->store(STRING_WITH_LEN("POS"),
        system_charset_info);
    tables->table->field[2]->store(pos_buf, strlen(pos_buf),
        system_charset_info);
    ret = schema_table_store_record(thd, tables->table);
    ...
    tables->table->field[0]->store(STRING_WITH_LEN("BINLOG"),
        system_charset_info);
    tables->table->field[1]->store(STRING_WITH_LEN("GTID"),
        system_charset_info);
...
static ST_FIELD_INFO i_s_rocksdb_generic_info_fields_info[] =
{
...
static int i_s_rocksdb_generic_info_init(void *p)
{
  ST_SCHEMA_TABLE *schema;

  DBUG_ENTER("i_s_rocksdb_generic_info_init");

  schema= reinterpret_cast<ST_SCHEMA_TABLE*>(p);

  schema->fields_info= i_s_rocksdb_generic_info_fields_info;
  schema->fill_table= i_s_rocksdb_generic_info_fill_table;
...
struct st_mysql_plugin i_s_rocksdb_generic_info=
{
  MYSQL_INFORMATION_SCHEMA_PLUGIN,
  &i_s_rocksdb_info,
  "ROCKSDB_GENERIC_INFO",
  "Facebook",
  "RocksDB generic info",
  PLUGIN_LICENSE_GPL,
  i_s_rocksdb_generic_info_init,
  i_s_rocksdb_deinit,
  0x0001,                             /* version number (0.1) */
  NULL,                               /* status variables */
  NULL,                               /* system variables */

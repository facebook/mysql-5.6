 ...
 private:
  ...
  bool parse_items() {
    ...
    for (auto &fname : m_param->columns) {
      ...
      Field *field = find_field_in_table(get_table(), fname.c_str(),
                                         fname.size(), false, &unused_index);
      if (!field) {
  ...
  inline Field *find_field(const std::string &field_name) {
    uint unused_index;
    Field *found = find_field_in_table(get_table(), field_name.c_str(),
                                       field_name.size(), false, &unused_index);
    if (!found) {
  ...
  class txn_wrapper {
   public:
    explicit txn_wrapper(THD *thd, TABLE_TYPE table_type)
      ...
      DBUG_ASSERT(m_tx && rdb_tx_started(m_tx, table_type));
    }

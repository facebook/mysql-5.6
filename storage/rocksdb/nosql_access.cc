...
class sql_cond : public base_cond {
 public:
  ...
  nosql_cond_value get_value(String *str, uint idx) const {
    DBUG_ASSERT(op_type == Item_func::IN_FUNC);
    auto in_func = static_cast<Item_func_in *>(cond_item);
...
class rpc_select_parser : public base_select_parser {
 public:
  explicit rpc_select_parser(THD *thd, const myrocks_select_from_rpc *param,
      ...
      : base_select_parser(thd, thd->lex->select_lex),
        m_param(param),
        m_columns(columns) {
 ...
 private:
  ...
  bool parse_items() {
    ...
    for (auto &fname : m_param->columns) {
      unsigned int cached_field_index = NO_CACHED_FIELD_INDEX;
      Field *field = find_field_in_table(
          get_table(), fname.c_str(), fname.size(), false, &cached_field_index);
      if (!field) {
  ...
  inline Field *find_field(const std::string &field_name) {
    unsigned int cached_field_index = NO_CACHED_FIELD_INDEX;
    Field *found =
        find_field_in_table(get_table(), field_name.c_str(), field_name.size(),
                            false, &cached_field_index);
    if (!found) {
...
bool select_exec::pack_index_tuple(uint key_part_no, Rdb_string_writer *writer,
  ...
  switch (field->type()) {
    case MYSQL_TYPE_LONGLONG: {
      DBUG_ASSERT(value.type == nosql_cond_value_type::UNSIGNED_INT);
    ...
    case MYSQL_TYPE_STRING: {
      DBUG_ASSERT(value.type == nosql_cond_value_type::STRING);

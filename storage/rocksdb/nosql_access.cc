...
class select_parser {
 public:
  ...
  bool INLINE_ATTR parse() {
    ...
    // INTO OUTFILE
    // DUMP OUTFILE
    if (m_thd->lex->result->needs_file_privilege()) {
  ...
  __attribute__((unused)) THD *get_thd() const { return m_thd; }
  __attribute__((unused)) TABLE *get_table() const { return m_table; }
  __attribute__((unused)) SELECT_LEX *get_select_lex() const {
    return m_select_lex;
  }
  __attribute__((unused)) uint get_index() const { return m_index; }
  __attribute__((unused)) bool is_order_desc() const { return m_is_order_desc; }
  __attribute__((unused)) Field *const *get_field_list() const {
    return m_field_list;
  }
  __attribute__((unused)) const sql_cond *get_cond_list() const {
    return m_cond_list;
  }
  __attribute__((unused)) uint get_field_count() const { return m_field_count; }
  __attribute__((unused)) uint get_cond_count() const { return m_cond_count; }
  __attribute__((unused)) uint64_t get_select_limit() const {
    return m_select_limit;
  }
  __attribute__((unused)) uint64_t get_offset_limit() const {
    return m_offset_limit;
  }
  __attribute__((unused)) const char *get_error_msg() const {
    return m_error_msg;
  }
  __attribute__((unused)) bool use_unoptimized_field_type() const {
    return m_use_unoptimized_field_type;
  }
 ...
 private:
  ...
  bool parse_fields() {
    ...
    while ((item = li++)) {
      ...
      Field *field =
          find_field_in_table(m_table, name, strlen(name), false,
      ...
      if (!m_use_unoptimized_field_type) {
        switch (field_type) {
          ...
          case MYSQL_TYPE_STRING:
            break;
          default: { m_use_unoptimized_field_type = true; }
        }
      }
    ...
    for (uint i = 0; i < m_select_lex->order_list.elements; ++i) {
      ...
      if (is_first) {
        ...
        for (uint j = 0; j < key->actual_key_parts; ++j) {
          if (check_field_name_match(key->key_part[j].field, field_item->field_name)) {
            cur_index = j;
            break;
          }
        }
        ...
        if (!check_field_name_match(key->key_part[cur_index].field, field_item->field_name)) {
  ...
  bool inline parse_cond(Item_func *func) {
    ...
    Field *found =
        find_field_in_table(m_table, field_name, strlen(field_name),
  ...
  bool parse_where() {
    ...
    if (where_type == Item::COND_ITEM) {
      ...
      Item_cond_and *and_cond =
          static_cast<Item_cond_and *>(where);
...
bool rocksdb_handle_single_table_select(THD *thd, SELECT_LEX *select_lex) {
  ...
  if (protocol->store(str.c_str(), str.length(), system_charset_info)) {
    return true;
  }

  ...
  const rocksdb::Slice& key)
{
  ...
  if (m_last_stats == nullptr || m_last_stats->m_gl_index_id != gl_index_id)
  {
    ...
    if (m_ddl_manager)
    {
      // find() returns a reference, but m_keydef is not a reference, so this
      // should increment the usage counter on the Rdb_key_def
      m_keydef = m_ddl_manager->find(gl_index_id);
      if (m_keydef)
      {
  ...
  rocksdb::EntryType type, uint64_t file_size)
{
  ...
  if (m_keydef && ShouldCollectStats())
  {
    std::size_t column = 0;
    bool new_key = true;

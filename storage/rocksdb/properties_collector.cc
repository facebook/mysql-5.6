...
void Rdb_tbl_prop_coll::CollectStatsForRow(
  ...
  m_gl_index_id.cf_id = m_cf_id;
  m_gl_index_id.index_id = read_big_uint4((const uchar*)key.data());

  if (m_stats.empty() || m_gl_index_id != m_stats.back().m_gl_index_id) {
    m_keydef = nullptr;
    // starting a new table
    // add the new element into m_stats
    m_stats.push_back(Rdb_index_stats(m_gl_index_id));
    if (m_ddl_manager) {
      m_keydef = m_ddl_manager->get_copy_of_keydef(m_gl_index_id);
    }
    if (m_keydef) {

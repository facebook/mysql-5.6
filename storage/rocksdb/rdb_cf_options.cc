                              ...
                              std::string* key)
{
  ...
  if (end_pos == beg_pos)
  {
    fprintf(stderr, "No column family found\n");
    return false;
  }
                         ...
                         std::string* options)
{
  ...
  {
    fprintf(stderr, "Invalid cf options, '{' expected\n");
    return false;
  }
  ...
  while (*pos < input.size())
  {
    ...
    if (input[*pos] == '}' && --brace_count == 0)
    {
      ++(*pos);  // Move past the last closing curly brace
      *options = input.substr(beg_pos, end_pos - beg_pos + 1);
      return true;
    }
  ...
  // Generate an error.
  fprintf(stderr, "Mismatched cf options, '}' expected\n");
  return false;
}
...
bool Cf_options::SetOverride(const std::string &override_config)
{
  ...
  while (pos < override_config.size())
  {
    ...
    {
      fprintf(stderr, "Duplicate entry for %s in override options", cf.c_str());
      return false;
    }
    ...
    {
      fprintf(stderr, "Invalid cf config for %s in override options",
              cf.c_str());
      return false;
    }

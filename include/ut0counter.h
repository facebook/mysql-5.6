...
struct generic_indexer_t {
	/** Default constructor/destructor should be OK. */
        ...
        size_t offset(size_t index) const UT_NOTHROW {
...
struct get_sched_indexer_t : public generic_indexer_t<Type, N> {
	...
	size_t get_rnd_index() const UT_NOTHROW {
...
public:
	...
	void inc() UT_NOTHROW { add(1); }

	/** If you can't use a good index id.
	* @param n  - is the amount to increment */
	void add(Type n) UT_NOTHROW {
	...
	void add(size_t index, Type n) UT_NOTHROW {
	...
	void dec() UT_NOTHROW { sub(1); }

	/** If you can't use a good index id.
	* @param - n is the amount to decrement */
	void sub(Type n) UT_NOTHROW {
		size_t	i = m_policy.offset(m_policy.get_rnd_index());
	...
	void sub(size_t index, Type n) UT_NOTHROW {
		size_t	i = m_policy.offset(index);
	...
	operator Type() const UT_NOTHROW {
		Type	total = 0;

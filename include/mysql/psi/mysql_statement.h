...
#ifdef HAVE_PSI_STATEMENT_INTERFACE
#define MYSQL_SNAPSHOT_STATEMENT(LOCKER, DA) inline_mysql_snapshot_statement(LOCKER, DA)
#else
...
static inline struct PSI_statement_locker *inline_mysql_start_statement(
   ...
   * capture delta and understand the impact of the queries in shorter time intervals
   */
  if (likely(locker != nullptr)) {

...
rm -rf $backup_dir/*
rm -rf $dest_data_dir/*
$MYSQL_INNOBACKUPEX --slave-info $defaults_file_opt --defaults-group=mysqld.1 $ibbackup_opt $backup_dir > ${MYSQL_TMP_DIR}/xtrabackup_copy_log 2>&1
...
# Check if gtid was printed
if grep -q "gtid_executed [0-9a-f\-]*:1-3" ${MYSQL_TMP_DIR}/xtrabackup_copy_log
then echo "gtid found";
else echo "gtid not found";
fi

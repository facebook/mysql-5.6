...
#
# The id space is split into two sections: exclusive and shared. Each loader
# thread owns some part of the exclusive section which it maintains complete
# information on insert/updates/deletes. Since this section is only modified by
# one thread, the thread can maintain an accurate picture of all changes. The
# shared section contains rows which multiple threads can update/delete/insert.
# For checking purposes, the request_id is used to determine if a row is
# consistent with a committed transaction.
#
# Checker threads run continuously to verify the checksums on the rows and to
# verify the zero_sum column sums up to zero at any point in time.
...
def wait_for_workers(workers, min_active = 0):
  ...
  min_active += 1
  try:
    while threading.active_count() > min_active and not TEST_STOP:
      time.sleep(1)
...
class WorkerThread(threading.Thread):
  ...
  def run(self):
    ...
    try:
      ...
      logging.info("Total run time: %.2f s" % (time.time() - self.start_time))
      self.finish()
      ...
      try:
        ...
        if self.con:
          ...
          if self.isolation_level:
            self.cur.execute("SET SESSION tx_isolation = %s" %
...
class PopulateWorker(WorkerThread):
  def __init__(self, id, start_id, num_to_add):
    WorkerThread.__init__(self, 'populate-%d' % id)
    self.id = id
...
class LoadGenWorker(WorkerThread):
  ...
  def __init__(self, id):
    WorkerThread.__init__(self, 'loader-%02d' % id)
    self.id = id
    ...
    self.start_id = id * OPTIONS.ids_per_loader
  ...
  def finish(self):
    logging.info("loop_num:%d, total time: %.2f s" % \
  ...
  def populate_id_map(self):
    ...
    REQ_ID_COL = 1
    stmt = "SELECT id, request_id FROM %s WHERE id = %d"

    # the start_id is used for tracking active transactions, so the row needs
    # to exist
    id = self.start_id
    execute(self.cur, stmt % (self.table, id))
    if self.cur.rowcount > 0:
      request_id = self.cur.fetchone()[REQ_ID_COL]
    else:
      request_id = get_next_request_id()
      execute(self.cur, gen_insert(self.table, id, self.id, request_id, 0))
      self.con.commit()

    self.id_map.append(request_id)

    self.cur_txn = {id:request_id}
    ...
    for id in xrange(self.start_id + 1, self.start_id + self.num_id):
      execute(self.cur, stmt % (self.table, id))
      if self.cur.rowcount == 0:
  ...
  def apply_cur_txn_changes(self):
    ...
    for id in self.cur_txn:
      if id < self.start_id + self.num_id:
        assert id >= self.start_id
        self.id_map[id - self.start_id] = self.cur_txn[id]
    self.cur_txn_state = self.TXN_COMMITTED
  ...
  def verify_data(self):
    ...
    if self.cur_txn_state == self.TXN_COMMIT_STARTED:
      assert request_id >= 0
      id = self.start_id
      stmt = "SELECT id, request_id FROM %s where id = %d" % (self.table, id)
      execute(self.cur, stmt)
      if (self.cur.rowcount == 0):
        raise TestError("Fetching start_id %d via '%s' returned no data! " \
                        "This row should never be deleted!" % (id, stmt))
      REQUEST_ID_COL = 1
      res = self.cur.fetchone()
      if res[REQUEST_ID_COL] == self.cur_txn[id]:
    ...
    while id < max_map_id:
      if (row_count == 0):
        ...
        execute(self.cur,
          "SELECT id, request_id FROM %s where id >= %d and id < %d \
           ORDER BY id LIMIT %d" \
          % (self.table, id, max_map_id, num_rows_to_check))
      ...
      while id < next_id:
        ...
        # that value.
        if self.id_map[id - self.start_id] >= 0:
          raise ValidateError("Row id %d was not found in table, but " \
                              "id_map has it at request_id %d" % \
                              (id, self.id_map[id - self.start_id]))
        id += 1

      if id == max_map_id:
        break

      if (self.id_map[id - self.start_id] != res[REQ_ID_COL]):
        raise ValidateError("Row id %d has req id %d, but %d is the " \
                            "expected value!" % \
                            (id, res[REQ_ID_COL],
                             self.id_map[id - self.start_id]))
      id += 1
  ...
  def execute_one(self):
    ...
    id = random.randint(self.start_id, self.start_id + self.num_id - 1)
    for i in xrange(1, num_rows):
      id = random.randint(self.start_share_id, self.max_id + self.num_id - 1)
      if id >= self.max_id:
        id -= self.max_id - self.start_id
      if ids.count(id) == 0:
        ids.append(id)
    ...
    for id in ids[:-1]:
      ...
      #
      # TODO -- Add an action that deletes and inserts a row
      if id in ids_found:
        # for each row found, determine if it should be updated or deleted
        if action == 0:
          stmt = gen_delete(self.table, id)
          is_delete = True
          current_sum -= ids_found[id]
        else:
          stmt = gen_update(self.table, id, self.id, request_id, zero_sum)
          current_sum += zero_sum
      else:
        # if it does not exist, then determine if an insert should happen
        if action <= 1:
          stmt = gen_insert(self.table, id, self.id, request_id, zero_sum)
          current_sum += zero_sum
    ...
    ids.append(id)
    self.cur_txn[id] = request_id
    stmt = gen_insert_on_dup(self.table, id, self.id, request_id, -current_sum)
    execute(self.cur, stmt)
    if self.cur.rowcount == 0:
      raise TestError("Executing '%s' returned row count of 0!" % stmt)
  ...
  def runme(self):
    ...
    while self.loop_num < self.num_requests:
      try:
...
class CheckerWorker(WorkerThread):
  def __init__(self, id):
    WorkerThread.__init__(self, 'checker-%02d' % id)
    self.id = id
    self.rand = random.Random()
    self.rand.seed(id)
    self.max_id = OPTIONS.max_id
  ...
  def check_zerosum(self):
    ...
    if (random.randint(1, 10) < 10):
      ...
      while id < self.max_id:
        execute(self.cur, stmt % (self.table, id, num_rows_to_check))
        if self.cur.rowcount == 0:
          break

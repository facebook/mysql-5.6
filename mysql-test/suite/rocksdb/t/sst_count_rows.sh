...
while : ; do
  ...
  do
    # excluding system cf
    DELETED=`$sst_dump --command=scan --output_hex --file=$f | perl -ne 'print  if(/''(\d\d\d\d\d\d\d\d)/ && $1 >= 8)' | grep -e ": 0" -e ": 7" | wc -l`
    EXISTS=`$sst_dump --command=scan --output_hex --file=$f | perl -ne 'print  if(/''(\d\d\d\d\d\d\d\d)/ && $1 >= 8)' | grep ": 1" | wc -l`
    TOTAL_D=$(($TOTAL_D+$DELETED))
    TOTAL_E=$(($TOTAL_E+$EXISTS))
    # echo "${f##*/} $DELETED $EXISTS"
  done
  if [ $TOTAL_E != "0" ] && [ $TOTAL_D = "0" ]
  then
    break
  fi
  if [ $TOTAL_E != "0" ] && [ $TOTAL_D != "0" ] && [ $wait_for_no_more_deletes = "0" ]
  then
    break
  fi

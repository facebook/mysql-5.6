...
*/
static int my_lfind(LF_SLIST * volatile *head,
                         ...
                         LF_SLIST *node, LF_PINS *pins, uint flags)
{
  ...
  for (;;)
  {
    if (my_lfind(head, cs, key_comp, node->hashnr, node->key, node->keylen,
                 &cursor, pins) &&
                   ...
                   const uchar *key, uint keylen, LF_PINS *pins)
{
  ...
  for (;;)
  {
    if (!my_lfind(head, cs, key_comp, hashnr, key, keylen, &cursor, pins))
    {
      res= 1; /* not found */
      break;
    }
    else
    {
                           ...
                           (void *)(((intptr)cursor.next) | 1)))
      {
        ...
        else
        {
          ...
          */
          my_lfind(head, cs, key_comp, hashnr, key, keylen, &cursor, pins);
        }
        res= 0;
        break;
      }
...
{
  CURSOR cursor;
  int res= my_lfind(head, cs, key_comp, hashnr, key, keylen, &cursor, pins);
  if (res)
    _lf_pin(pins, 2, cursor.curr);
  _lf_unpin(pins, 0);
  _lf_unpin(pins, 1);
...
void *lf_hash_search(LF_HASH *hash, LF_PINS *pins, const void *key, uint keylen)
{
  ...
  if (*el == NULL && unlikely(initialize_bucket(hash, el, bucket, pins)))
    return MY_ERRPTR;
  found= my_lsearch(el, hash->charset, hash->key_comparator, my_reverse_bits(hashnr) | 1,
                    (uchar *)key, keylen, pins);
  lf_rwunlock_by_pins(pins);
